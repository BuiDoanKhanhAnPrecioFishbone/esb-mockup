...
**Not for visual artifacts or app changes.** If the user wants to save a JSX artifact Codex generated in chat, update a part of the app (dashboard, a session tab, the sidebar), or add a new route, that's `apply-to-mockup`'s job — not this one. If they want both — log the decision *and* update the UI to follow it — do this skill **first** (so the new context is the source of truth), then chain into `apply-to-mockup` as a second commit.
