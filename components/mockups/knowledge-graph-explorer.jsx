...
const NODES = [
  ...
  { id:"payment", label:"Payment Processing", type:"module", depth:1, parent:"eng", entries:12, verified:12, flagged:1, gaps:0, summary:"Covers Kafka pipeline, Stripe integration,\nwebhook verification, PCI compliance", provenance:[{name:"Minh L\u00ea",date:"Jun 2026",count:10},{name:"Thanh \u0110\u1ee9c",date:"Mar 2026",count:2}] },
  ...
  { id:"database", label:"Database & Migrations", type:"module", depth:1, parent:"eng", entries:9, verified:9, flagged:0, gaps:0, summary:"Cosmos DB partitioning, Flyway pipeline,\nmanual migration procedures", provenance:[{name:"Minh L\u00ea",date:"Jun 2026",count:7},{name:"Thanh \u0110\u1ee9c",date:"Mar 2026",count:2}] },
  ...
  { id:"monitor", label:"Monitoring", type:"module", depth:1, parent:"eng", entries:5, verified:5, flagged:0, gaps:0, summary:"Grafana dashboards, PagerDuty alerts,\nSLA definitions, escalation paths", provenance:[{name:"Minh L\u00ea",date:"Jun 2026",count:3},{name:"Thanh \u0110\u1ee9c",date:"Mar 2026",count:2}] },
  ...
  { id:"infra", label:"Infrastructure as Code", type:"module", depth:1, parent:"eng", entries:3, verified:3, flagged:0, gaps:0, summary:"Terraform modules, AKS cluster config,\nnetwork policies", provenance:[{name:"Thanh \u0110\u1ee9c",date:"Mar 2026",count:3}] },
...
export default function KnowledgeGraphExplorer({embedded=false}={}){
  ...
  const {visNodes,visEdges}=useMemo(()=>{
    ...
    // Make a hovered chat-reference node (and its parent) visible so its glow can render even if its module is collapsed.
    if(refHover){show.add(refHover);const nd=NODES.find(nn=>nn.id===refHover);if(nd&&nd.parent){show.add(nd.parent);}}
          ...
          <svg ref={svgRef} width={dim.w} height={dim.h} className="w-full h-full" style={{touchAction:'none'}} onPointerMove={onMove} onPointerUp={onUp} onPointerDown={onBgDown}>
            <g transform={`translate(${pan.x},${pan.y}) scale(${pan.s})`}>
              {visEdges.map((e,i)=>{const s=nm[e.from],t=nm[e.to];if(!s||!t)return null;const sn=NODES.find(n=>n.id===e.from),tn=NODES.find(n=>n.id===e.to);const sr=nodeR(sn||{}),tr=nodeR(tn||{});const dx=t.x-s.x,dy=t.y-s.y,d=Math.sqrt(dx*dx+dy*dy)||1;const x1=s.x+(dx/d)*sr,y1=s.y+(dy/d)*sr,x2=t.x-(dx/d)*tr,y2=t.y-(dy/d)*tr;const hi=isHi(e.from)&&isHi(e.to);
                ...
                const eOp=hi?0.4:(focusIds||selected||hasActiveFilter)?0.08:0.4;return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={e.type==="cross"?"#c4b5fd":"#d4d4d8"} strokeWidth={e.type==="cross"?0.8:0.6} strokeDasharray={e.type==="cross"?"4,3":"none"} opacity={eOp} style={{transition:'opacity 0.15s'}}/>;
              })}
