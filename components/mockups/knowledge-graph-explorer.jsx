...
import { Send, Sparkles, X, ChevronRight, Flag, Filter, BarChart3, CheckCircle2, XCircle } from "lucide-react";

/* ART-EEP Consumer Plane — Knowledge Graph Explorer
   15 decisions + HITL report + filters + 0% hide + gap resolution
   Gap actions: "Mark as resolved" (gap→verified) + "Dismiss gap" (gap→draft, removed from gap list) */
...
export default function KnowledgeGraphExplorer({embedded=false}={}){
  ...
  const nodeFill=useCallback((n)=>isGap(n.id)?"#fef9c3":resolvedGaps.has(n.id)?"#ecfdf5":n.type==="dept"||n.type==="system"?"#f4f4f5":"#f5f3ff",[isGap,resolvedGaps]);
  const nodeStroke=useCallback((n)=>isGap(n.id)?"#facc15":resolvedGaps.has(n.id)?"#6ee7b7":n.type==="dept"||n.type==="system"?"#d4d4d8":"#c4b5fd",[isGap,resolvedGaps]);
  ...
  const entryPasses=(nd)=>{
    ...
    if(fGaps==="yes"&&!isGap(nd.id))return false;
      ...
      <div className="flex-1 min-h-0 flex gap-2">
        <div ref={boxRef} className={`${selected?'w-3/5':'w-full'} bg-gray-50 rounded-lg border border-gray-200 relative overflow-hidden transition-all duration-200`} style={{cursor:panning?'grabbing':dragId?'grabbing':'grab'}}>
            ...
            <g transform={`translate(${pan.x},${pan.y}) scale(${pan.s})`}>
              ...
              {nodes.map(node=>{const r=nodeR(node),hi=isHi(node.id),isSel=selected===node.id,isHov=hovered===node.id,isMod=node.type==="module",isExp=expanded.has(node.id),isReported=reported.has(node.id);const nf=nodeFill(node);const ns=nodeStroke(node);
                ...
                return <g key={node.id} transform={`translate(${node.x||0},${node.y||0})`} style={{cursor:'pointer'}} onPointerDown={e=>onDown(e,node.id)} onPointerEnter={e=>{setHovered(node.id);setTipPos({x:e.clientX,y:e.clientY});}} onPointerLeave={()=>setHovered(null)} onClick={()=>onClickNode(node)}>
                  ...
                  {resolvedGaps.has(node.id)&&<g transform={`translate(${r-2},${-r+2})`}><circle r="5.5" fill="#d1fae5" stroke="#6ee7b7" strokeWidth="1"/><text textAnchor="middle" dominantBaseline="central" fontSize="7" fill="#065f46" fontWeight="600">{"\u2713"}</text></g>}
                  {isReported&&!resolvedGaps.has(node.id)&&<g transform={`translate(${r-2},${-r+2})`}><circle r="5.5" fill="#fecaca" stroke="#f87171" strokeWidth="1"/><text textAnchor="middle" dominantBaseline="central" fontSize="7" fill="#991b1b" fontWeight="600">!</text></g>}
                  <text y={r+12} textAnchor="middle" fontSize={node.depth===0?11:node.depth===1?10:9} fontWeight={node.depth<=1?500:400} fill={dismissedGaps.has(node.id)?"#a1a1aa":"#27272a"} style={{pointerEvents:'none',userSelect:'none',textDecoration:dismissedGaps.has(node.id)?"line-through":"none"}}>{node.label.length>20?node.label.slice(0,18)+"\u2026":node.label}</text>
                </g>;})}
            </g>
          </svg>
          {hovered&&(()=>{const node=NODES.find(n=>n.id===hovered);if(!node)return null;const rect=boxRef.current?.getBoundingClientRect();if(!rect)return null;const tx=tipPos.x-rect.left+14,ty=tipPos.y-rect.top-8,flip=tx+240>rect.width;const nf=nodeFill(node);const ns=nodeStroke(node);const gs=gapStatus(node.id);
            return <div className="absolute pointer-events-none z-50" style={{left:flip?tx-254:tx,top:Math.max(4,Math.min(ty,rect.height-120))}}><div className="bg-white border border-gray-200 rounded-lg shadow-md p-2.5 w-[230px]" style={{borderLeft:`3px solid ${ns}`}}><div className="flex items-center gap-1.5 mb-1"><span className="text-[10px] font-medium px-1.5 py-0.5 rounded" style={{background:nf,border:`1px solid ${ns}`,color:isGap(node.id)?"#854d0e":gs==="resolved"?"#065f46":node.type==="dept"||node.type==="system"?"#52525b":"#5b21b6"}}>{gs==="resolved"?"Resolved":gs==="dismissed"?"Dismissed":node.type==="dept"?"Department":node.type==="module"?"Module":node.type==="system"?"System":isGap(node.id)?"Entry (gap)":"Entry"}</span>{reported.has(node.id)&&!gs&&<span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-rose-50 text-rose-700 border border-rose-200">Reported</span>}</div><p className="text-xs font-medium text-gray-900 mb-0.5">{node.label}</p>{node.type==="module"&&<p className="text-[10px] text-gray-500">{node.entries}{" entries \u00b7 "}{node.verified}{" verified \u00b7 "}{node.draft}{" draft"}{node.gaps>0?` \u00b7 ${node.gaps} gaps`:""}</p>}{node.type==="entry"&&<p className="text-[10px] text-gray-500">{gs==="resolved"?"Verified (was gap)":gs==="dismissed"?"Dismissed (false positive)":node.status}{isGap(node.id)?" \u00b7 has gap":""}</p>}{node.type==="module"&&<p className="text-[10px] text-violet-600 mt-1">{"Click to "}{expanded.has(node.id)?"collapse":"expand"}</p>}</div></div>;})()}
          <div className="absolute bottom-2 left-2 flex items-center gap-3 bg-white/80 backdrop-blur-sm rounded px-2 py-1"><span className="flex items-center gap-1 text-[9px] text-gray-400"><span className="w-2 h-2 rounded-full bg-gray-200 border border-gray-300 inline-block"></span>Structural</span><span className="flex items-center gap-1 text-[9px] text-gray-400"><span className="w-2 h-2 rounded-full bg-violet-100 border border-violet-300 inline-block"></span>Knowledge</span><span className="flex items-center gap-1 text-[9px] text-gray-400"><span className="w-2 h-2 rounded-full bg-yellow-100 border border-yellow-400 inline-block"></span>Gap</span><span className="flex items-center gap-1 text-[9px] text-gray-400"><span className="w-2 h-2 rounded-full bg-emerald-100 border border-emerald-400 inline-block"></span>Resolved</span><span className="flex items-center gap-1 text-[9px] text-gray-400"><span className="w-2 h-2 rounded-full bg-rose-100 border border-rose-400 inline-block"></span>Reported</span><span className="text-[9px] text-gray-300">|</span><span className="text-[9px] text-gray-400">Solid = hierarchy</span><span className="text-[9px] text-gray-400">Dashed = cross-link</span></div>
        </div>
        {selData&&<div className="w-2/5 bg-white border border-gray-200 rounded-lg overflow-y-auto" style={{borderLeft:`3px solid ${reported.has(selData.id)&&!selGapStatus?"#f87171":nodeStroke(selData)}`}}><div className="p-3"><div className="flex items-start justify-between mb-2"><div><div className="flex items-center gap-1.5 mb-1"><span className="text-[10px] font-medium px-1.5 py-0.5 rounded" style={{background:nodeFill(selData),border:`1px solid ${nodeStroke(selData)}`,color:selIsGap?"#854d0e":selGapStatus==="resolved"?"#065f46":selData.type==="dept"||selData.type==="system"?"#52525b":"#5b21b6"}}>{selGapStatus==="resolved"?"Resolved":selGapStatus==="dismissed"?"Dismissed":selData.type==="dept"?"Department":selData.type==="module"?"Module":selData.type==="system"?"System":selIsGap?"Entry (gap)":"Entry"}</span>{reported.has(selData.id)&&!selGapStatus&&<span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-rose-50 text-rose-700 border border-rose-200">1 pending correction</span>}</div><h3 className="text-sm font-semibold text-gray-900">{selData.label}</h3></div><button onClick={()=>{setSelected(null);setReportingNode(null);setReportText("");}} className="text-gray-400 hover:text-gray-600 cursor-pointer"><X className="w-4 h-4"/></button></div>
            {selGapStatus==="resolved"&&<div className="rounded-md bg-emerald-50 border border-emerald-200 px-3 py-2 mb-3 flex items-start gap-2"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0 mt-0.5"/><div><p className="text-[11px] text-emerald-800 font-medium">Gap resolved</p><p className="text-[10px] text-emerald-600">{"Marked as resolved by H\u00e0 Vy \u00b7 Knowledge has been captured."}</p></div></div>}
            {selGapStatus==="dismissed"&&<div className="rounded-md bg-gray-50 border border-gray-200 px-3 py-2 mb-3 flex items-start gap-2"><XCircle className="w-3.5 h-3.5 text-gray-500 shrink-0 mt-0.5"/><div><p className="text-[11px] text-gray-700 font-medium">Gap dismissed</p><p className="text-[10px] text-gray-500">{"Dismissed by H\u00e0 Vy \u00b7 AI flagged this incorrectly."}</p></div></div>}
            ...
            {selData.type==="entry"&&!selGapStatus&&<div className="flex items-center gap-1.5 mb-2"><span className={`text-[10px] px-1.5 py-0.5 rounded border ${selData.status==="verified"?"bg-violet-50 text-violet-700 border-violet-200":"bg-gray-50 text-gray-600 border-gray-200"}`}>{selData.status==="verified"?"Verified":"Draft"}</span>{selIsGap&&<span className="text-[10px] px-1.5 py-0.5 rounded bg-yellow-50 text-yellow-700 border border-yellow-200">Has gap</span>}</div>}
            <p className={`text-[11px] text-gray-600 whitespace-pre-wrap leading-relaxed mb-3 ${selGapStatus==="dismissed"?"line-through text-gray-400":""}`}>{selData.summary}</p>
            ...
            {childEntries.length>0&&<div className="border-t border-gray-100 pt-2 mb-2"><p className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1.5">Entries</p>{childEntries.map(ce=>{const ceGs=gapStatus(ce.id);const ceIsGap=isGap(ce.id);return <button key={ce.id} onClick={()=>{setSelected(ce.id);setReportingNode(null);setReportText("");}} className="w-full flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-50 text-left cursor-pointer transition-colors"><span className="w-2 h-2 rounded-full flex-shrink-0" style={{backgroundColor:reported.has(ce.id)&&!ceGs?"#f87171":ceGs==="resolved"?"#6ee7b7":ceGs==="dismissed"?"#d4d4d8":ceIsGap?"#facc15":ce.status==="verified"?"#a78bfa":"#d4d4d8"}}></span><span className={`text-[11px] flex-1 truncate ${ceGs==="dismissed"?"text-gray-400 line-through":"text-gray-800"}`}>{ce.label}</span>{ceGs==="resolved"?<span className="text-[9px] px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-600">Resolved</span>:ceGs==="dismissed"?<span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-400">Dismissed</span>:reported.has(ce.id)?<span className="text-[9px] px-1.5 py-0.5 rounded bg-rose-50 text-rose-600">Reported</span>:<span className={`text-[9px] px-1.5 py-0.5 rounded ${ce.status==="verified"?"bg-violet-50 text-violet-600":"bg-gray-50 text-gray-500"}`}>{ce.status==="verified"?"Verified":"Draft"}</span>}</button>})}</div>}
            ...
            <div className="border-t border-gray-100 pt-2 flex flex-col gap-1.5">
              ...
              {selIsGap&&<><button onClick={()=>onResolveGap(selData.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-700 text-[11px] font-medium hover:bg-emerald-100 transition-colors cursor-pointer"><CheckCircle2 className="w-3 h-3"/>Mark as resolved</button><button onClick={()=>onDismissGap(selData.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-white border border-gray-200 text-gray-500 text-[11px] font-medium hover:bg-gray-50 hover:border-gray-300 transition-colors cursor-pointer"><XCircle className="w-3 h-3"/>Dismiss gap</button></>}
