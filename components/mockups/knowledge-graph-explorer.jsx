...
import { Search, Send, Sparkles, X, ChevronRight } from "lucide-react";

/* ART-EEP Consumer Plane — Knowledge Graph Explorer
   12 locked decisions applied:
   1. Two entry points (sidebar=dept-first, session=person-centered)
   2. Department nodes as default, Engineering only for POC
   3. 1-2 past handovers as graph history
   4. Modules as primary structure, people as provenance
   5. Equal visual weight, Draft/Verified only, no time decay
   6. Static mock chat
   7. Minimal circles, color by role, size by depth
   8. Solid=hierarchy, dashed=cross-links, type in panel
   9. Purple=knowledge, gray=structural, yellow=gaps
   10. Chat bar bottom, always visible
   11. Hover tooltip + side panel + "Ask about this"
   12. No readiness %, module shows count+verified/draft+gaps
*/
...
function initPos(nodes, w, h) {
  ...
  const d = nodes.find(n=>n.depth===0);
  if(d){d.x=cx;d.y=cy;}
  ...
  nodes.filter(n=>n.depth===2).forEach(n=>{
    const p=nodes.find(pp=>pp.id===n.parent||(pp.type==="system"&&n.type==="system"));
    if(!p){n.x=cx+(Math.random()-.5)*200;n.y=cy+(Math.random()-.5)*200;return;}
    const sibs=nodes.filter(s=>s.parent===n.parent&&s.depth===2);
    const idx=sibs.indexOf(n);
    const ba=Math.atan2(p.y-cy,p.x-cx);
    const spread=sibs.length>1?Math.PI*0.6:0;
    const a=ba-spread/2+(sibs.length>1?(idx/(sibs.length-1))*spread:0);
    n.x=p.x+Math.cos(a)*90;n.y=p.y+Math.sin(a)*90;
  });
  const kv=nodes.find(n=>n.id==="s-keyvault");
  const pd=nodes.find(n=>n.id==="s-pagerduty");
  if(kv){kv.x=cx+240;kv.y=cy-80;}
  if(pd){pd.x=cx+240;pd.y=cy+120;}
}

function sim(nodes,edges,w,h){
  const a=0.2,rep=1800,sp=0.03,sl=100,ce=0.006,da=0.55;
  const m={};nodes.forEach(n=>{m[n.id]=n;});
  for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){
    const na=nodes[i],nb=nodes[j],dx=nb.x-na.x,dy=nb.y-na.y,d=Math.sqrt(dx*dx+dy*dy)||1;
    const f=rep/(d*d)*a,fx=(dx/d)*f,fy=(dy/d)*f;
...
export default function KnowledgeGraphExplorer({embedded=false}={}){
  ...
  const [pan,setPan]=useState({x:0,y:0,s:1});
  ...
  const {visNodes,visEdges}=useMemo(()=>{
    const show=new Set();
    NODES.filter(n=>n.depth<=1).forEach(n=>show.add(n.id));
    NODES.filter(n=>n.depth===2).forEach(n=>{
      if(n.type==="system"){if(expanded.size>0)show.add(n.id);return;}
      const p=NODES.find(pp=>pp.id===n.parent);
      if(p&&expanded.has(p.id))show.add(n.id);
    });
  ...
  useEffect(()=>{
    ...
    if(copies.some(n=>n.x===0&&n.y===0))initPos(copies,dim.w,dim.h);
    nodesRef.current=copies;
    let run=true;
    const step=()=>{if(!run)return;sim(nodesRef.current,visEdges,dim.w,dim.h);setTick(t=>t+1);animRef.current=requestAnimationFrame(step);};
  ...
  useEffect(()=>{
    ...
    const ro=new ResizeObserver(([e])=>{const{width,height}=e.contentRect;if(width>0)setDim({w:width,h:height});});
  ...
  const onMove=useCallback((e)=>{
    if(dragId){const n=nodesRef.current.find(x=>x.id===dragId);if(n&&svgRef.current){const r=svgRef.current.getBoundingClientRect();n.x=(e.clientX-r.left-pan.x)/pan.s;n.y=(e.clientY-r.top-pan.y)/pan.s;n.vx=0;n.vy=0;setTick(t=>t+1);}}
    else if(panning){setPan(p=>({...p,x:panRef.current.px+e.clientX-panRef.current.x,y:panRef.current.py+e.clientY-panRef.current.y}));}
    if(hovered)setTipPos({x:e.clientX,y:e.clientY});
  },[dragId,panning,hovered,pan]);
  ...
  const onClickNode=useCallback((node)=>{
    if(node.type==="module"){setExpanded(prev=>{const n=new Set(prev);n.has(node.id)?n.delete(node.id):n.add(node.id);return n;});}
    setSelected(s=>s===node.id?null:node.id);
  },[]);

  const onChip=useCallback((chip)=>{
    const mods=new Set();
    chip.focus.forEach(id=>{const nd=NODES.find(n=>n.id===id);if(nd?.parent){mods.add(nd.parent);}});
    setExpanded(prev=>new Set([...prev,...mods]));
    setChatFocus(chip.focus);setChatResponse(chip.response);setSelected(null);
  },[]);

  const onAsk=(nodeId)=>{
    const nd=NODES.find(n=>n.id===nodeId);if(!nd)return;
    const q=nd.type==="module"?`What are the risks in ${nd.label}?`:`Tell me about ${nd.label}`;
    setChatInput(q);
    const relEntries=NODES.filter(n=>n.parent===nodeId).map(n=>n.id);
    if(relEntries.length>0){setChatFocus(relEntries);setChatResponse(`${nd.label} contains ${nd.entries||relEntries.length} entries. ${nd.gaps>0?`${nd.gaps} knowledge gap${nd.gaps>1?"s":""} need attention.`:""} ${nd.summary||""}`);}
    else{setChatFocus([nodeId]);setChatResponse(nd.summary||"No additional details available.");}
  };
    ...
    <div className={`${embedded?'':'p-4'} flex flex-col h-full min-h-0`} style={{fontFamily:"'Inter',system-ui,sans-serif"}}>
      <div className="flex items-center justify-between mb-2 flex-shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-semibold text-gray-900 leading-tight">Knowledge Graph</h2>
            <p className="text-[11px] text-gray-500">Engineering · {NODES.filter(n=>n.type==="module").length} modules · {NODES.filter(n=>n.type==="entry").length} entries · {EDGES.length} relationships</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={()=>{setExpanded(new Set());setPan({x:0,y:0,s:1});setSelected(null);setChatFocus(null);setChatResponse("");setChatInput("");}} className="px-2.5 py-1 text-[11px] font-medium text-gray-600 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors cursor-pointer">Reset</button>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 flex gap-2">
        <div ref={boxRef} className={`${selected?'w-3/5':'w-full'} bg-gray-50 rounded-lg border border-gray-200 relative overflow-hidden transition-all duration-200`} style={{cursor:panning?'grabbing':dragId?'grabbing':'grab'}}>
          <svg ref={svgRef} width={dim.w} height={dim.h} className="w-full h-full" style={{touchAction:'none'}} onPointerMove={onMove} onPointerUp={onUp} onPointerDown={onBgDown} onWheel={onWheel}>
            <g transform={`translate(${pan.x},${pan.y}) scale(${pan.s})`}>
              {visEdges.map((e,i)=>{const s=nm[e.from],t=nm[e.to];if(!s||!t)return null;const sn=NODES.find(n=>n.id===e.from),tn=NODES.find(n=>n.id===e.to);const sr=nodeR(sn||{}),tr=nodeR(tn||{});const dx=t.x-s.x,dy=t.y-s.y,d=Math.sqrt(dx*dx+dy*dy)||1;const x1=s.x+(dx/d)*sr,y1=s.y+(dy/d)*sr,x2=t.x-(dx/d)*tr,y2=t.y-(dy/d)*tr;const hi=isHi(e.from)&&isHi(e.to);
                return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={e.type==="cross"?"#c4b5fd":"#d4d4d8"} strokeWidth={e.type==="cross"?0.8:0.6} strokeDasharray={e.type==="cross"?"4,3":"none"} opacity={hi?0.4:0.06} style={{transition:'opacity 0.15s'}}/>;
              })}
              {nodes.map(node=>{const r=nodeR(node),hi=isHi(node.id),isSel=selected===node.id,isHov=hovered===node.id,isMod=node.type==="module",isExp=expanded.has(node.id);
          ...
          {hovered&&(()=>{const node=NODES.find(n=>n.id===hovered);if(!node)return null;const rect=boxRef.current?.getBoundingClientRect();if(!rect)return null;const tx=tipPos.x-rect.left+14,ty=tipPos.y-rect.top-8,flip=tx+240>rect.width;
            return <div className="absolute pointer-events-none z-50" style={{left:flip?tx-254:tx,top:Math.max(4,Math.min(ty,rect.height-120))}}>
              <div className="bg-white border border-gray-200 rounded-lg shadow-md p-2.5 w-[230px]" style={{borderLeft:`3px solid ${nodeStroke(node)}`}}>
                <div className="flex items-center gap-1.5 mb-1">
                  <span className="text-[10px] font-medium px-1.5 py-0.5 rounded" style={{background:nodeFill(node),border:`1px solid ${nodeStroke(node)}`,color:node.hasGap?"#854d0e":node.type==="dept"||node.type==="system"?"#52525b":"#5b21b6"}}>{node.type==="dept"?"Department":node.type==="module"?"Module":node.type==="system"?"System":node.hasGap?"Entry (gap)":"Entry"}</span>
                </div>
                <p className="text-xs font-medium text-gray-900 mb-0.5">{node.label}</p>
                {node.type==="module"&&<p className="text-[10px] text-gray-500">{node.entries} entries · {node.verified} verified · {node.draft} draft{node.gaps>0?` · ${node.gaps} gaps`:""}</p>}
                {node.type==="entry"&&<p className="text-[10px] text-gray-500">{node.status}{node.hasGap?" · has gap":""}</p>}
                {node.type==="module"&&<p className="text-[10px] text-violet-600 mt-1">Click to {expanded.has(node.id)?"collapse":"expand"}</p>}
              </div>
            </div>;
          })()}
          <div className="absolute bottom-2 left-2 flex items-center gap-3 bg-white/80 backdrop-blur-sm rounded px-2 py-1">
            <span className="flex items-center gap-1 text-[9px] text-gray-400"><span className="w-2 h-2 rounded-full bg-gray-200 border border-gray-300 inline-block"></span>Structural</span>
            <span className="flex items-center gap-1 text-[9px] text-gray-400"><span className="w-2 h-2 rounded-full bg-violet-100 border border-violet-300 inline-block"></span>Knowledge</span>
            <span className="flex items-center gap-1 text-[9px] text-gray-400"><span className="w-2 h-2 rounded-full bg-yellow-100 border border-yellow-400 inline-block"></span>Gap</span>
            <span className="text-[9px] text-gray-300">|</span>
            <span className="text-[9px] text-gray-400">Solid = hierarchy</span>
            <span className="text-[9px] text-gray-400">Dashed = cross-link</span>
          </div>
        </div>

        {selData&&<div className="w-2/5 bg-white border border-gray-200 rounded-lg overflow-y-auto" style={{borderLeft:`3px solid ${nodeStroke(selData)}`}}>
          <div className="p-3">
            <div className="flex items-start justify-between mb-2">
              <div>
                <span className="text-[10px] font-medium px-1.5 py-0.5 rounded inline-block mb-1" style={{background:nodeFill(selData),border:`1px solid ${nodeStroke(selData)}`,color:selData.hasGap?"#854d0e":selData.type==="dept"||selData.type==="system"?"#52525b":"#5b21b6"}}>{selData.type==="dept"?"Department":selData.type==="module"?"Module":selData.type==="system"?"System":selData.hasGap?"Entry (gap)":"Entry"}</span>
                <h3 className="text-sm font-semibold text-gray-900">{selData.label}</h3>
              </div>
              <button onClick={()=>setSelected(null)} className="text-gray-400 hover:text-gray-600 cursor-pointer"><X className="w-4 h-4"/></button>
            </div>
            {selData.type==="module"&&<>
              <div className="flex flex-wrap items-center gap-1.5 mb-2">
                <span className="text-[11px] text-gray-600">{selData.entries} entries</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-violet-50 text-violet-700 border border-violet-200">{selData.verified} verified</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-50 text-gray-600 border border-gray-200">{selData.draft} draft</span>
                {selData.gaps>0&&<span className="text-[10px] px-1.5 py-0.5 rounded bg-yellow-50 text-yellow-700 border border-yellow-200">{selData.gaps} gaps</span>}
              </div>
              {selData.provenance?.map((p,i)=><p key={i} className="text-[10px] text-gray-400">{p.name} · {p.date} · {p.count} entries</p>)}
            </>}
            {selData.type==="entry"&&<div className="flex items-center gap-1.5 mb-2">
              <span className={`text-[10px] px-1.5 py-0.5 rounded border ${selData.status==="verified"?"bg-violet-50 text-violet-700 border-violet-200":"bg-gray-50 text-gray-600 border-gray-200"}`}>{selData.status==="verified"?"Verified":"Draft"}</span>
              {selData.hasGap&&<span className="text-[10px] px-1.5 py-0.5 rounded bg-yellow-50 text-yellow-700 border border-yellow-200">Has gap</span>}
            </div>}
            <p className="text-[11px] text-gray-600 whitespace-pre-wrap leading-relaxed mb-3">{selData.summary}</p>
            {childEntries.length>0&&<>
              <div className="border-t border-gray-100 pt-2 mb-2"><p className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1.5">Entries</p>
                {childEntries.map(ce=><button key={ce.id} onClick={()=>setSelected(ce.id)} className="w-full flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-50 text-left cursor-pointer transition-colors">
                  <span className="w-2 h-2 rounded-full flex-shrink-0" style={{backgroundColor:ce.hasGap?"#facc15":ce.status==="verified"?"#a78bfa":"#d4d4d8"}}></span>
                  <span className="text-[11px] text-gray-800 flex-1 truncate">{ce.label}</span>
                  <span className={`text-[9px] px-1.5 py-0.5 rounded ${ce.status==="verified"?"bg-violet-50 text-violet-600":"bg-gray-50 text-gray-500"}`}>{ce.status==="verified"?"Verified":"Draft"}</span>
                </button>)}
              </div>
            </>}
            {selEdges.length>0&&<div className="border-t border-gray-100 pt-2 mb-2"><p className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1.5">Cross-links ({selEdges.filter(e=>e.type==="cross").length})</p>
              {selEdges.filter(e=>e.type==="cross").map((e,i)=>{const oid=e.from===selData.id?e.to:e.from;const o=NODES.find(n=>n.id===oid);if(!o)return null;
                return <button key={i} onClick={()=>{setSelected(oid);if(o.parent)setExpanded(prev=>new Set([...prev,o.parent]));}} className="w-full flex items-center gap-2 px-2 py-1.5 rounded hover:bg-violet-50 text-left cursor-pointer transition-colors">
                  <ChevronRight className="w-3 h-3 text-gray-400"/>
                  <span className="text-[11px] text-gray-800 flex-1">{o.label}</span>
                  {e.label&&<span className="text-[9px] text-gray-400">{e.label}</span>}
                </button>;
              })}
            </div>}
            <div className="border-t border-gray-100 pt-2">
              <button onClick={()=>onAsk(selData.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-violet-50 border border-violet-200 text-violet-700 text-[11px] font-medium hover:bg-violet-100 transition-colors cursor-pointer">
                <Sparkles className="w-3 h-3"/>Ask about this {selData.type}
              </button>
            </div>
          </div>
        </div>}
      ...
      <div className="mt-2 flex-shrink-0 bg-white border border-gray-200 rounded-lg px-3 py-2">
        ...
        <div className="flex items-center gap-1.5 mb-1.5">
          <span className="flex items-center gap-1 px-2 py-0.5 bg-violet-50 rounded border border-violet-100 text-[10px] font-medium text-violet-700"><Sparkles className="w-2.5 h-2.5"/>AI Copilot</span>
          {CHIPS.map((c,i)=><button key={i} onClick={()=>onChip(c)} className={`px-2 py-0.5 text-[10px] font-medium rounded-full border transition-colors cursor-pointer ${chatFocus&&chatResponse===c.response?"bg-violet-600 text-white border-violet-600":"bg-white text-gray-600 border-gray-200 hover:border-violet-300 hover:text-violet-700"}`}>{c.label}</button>)}
        </div>
        <div className="flex items-center gap-2">
          <input type="text" value={chatInput} onChange={e=>setChatInput(e.target.value)} placeholder="Ask about the knowledge graph..." className="flex-1 px-2.5 py-1.5 text-[11px] border border-gray-200 rounded-md bg-gray-50 focus:outline-none focus:ring-1 focus:ring-violet-500/20 focus:border-violet-300 text-gray-700 placeholder:text-gray-400"/>
          <button className="px-3 py-1.5 bg-violet-600 text-white rounded-md text-[11px] font-medium hover:bg-violet-700 transition-colors cursor-pointer flex items-center gap-1"><Send className="w-3 h-3"/>Send</button>
        </div>
      </div>
    </div>
  );
