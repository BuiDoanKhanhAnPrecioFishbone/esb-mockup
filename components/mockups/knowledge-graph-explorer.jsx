...
/* ART-EEP Consumer Plane — Knowledge Graph Explorer
   12 locked decisions + from-session entry point (?prompt=minh-le)
*/
...
export default function KnowledgeGraphExplorer({embedded=false}={}){
  ...
  useEffect(()=>{
    ...
    if(prompt==="minh-le"){
      const minhMods=NODES.filter(n=>n.type==="module"&&n.provenance?.some(p=>p.name.includes("Minh"))).map(n=>n.id);
      const minhEntries=NODES.filter(n=>n.depth===2&&n.type==="entry"&&minhMods.includes(n.parent)).map(n=>n.id);
      setExpanded(new Set(minhMods));setChatFocus([...minhMods,...minhEntries]);
      setChatInput("Show me Minh L\u00ea's contributions");
      setChatResponse("Minh L\u00ea contributed 42 entries across 6 modules (Payment, Auth, Database, CI/CD, Monitoring, Rate Limiting). 5 knowledge gaps remain across 3 modules. All entries are from his Jun 2026 handover session.");
  ...
  const onClickNode=useCallback((node)=>{if(node.type==="module"){setExpanded(prev=>{const n=new Set(prev);n.has(node.id)?n.delete(node.id):n.add(node.id);return n;});}setSelected(s=>s===node.id?null:node.id);},[]);
  ...
  const onAsk=(nodeId)=>{const nd=NODES.find(n=>n.id===nodeId);if(!nd)return;const q=nd.type==="module"?`What are the risks in ${nd.label}?`:`Tell me about ${nd.label}`;setChatInput(q);const relEntries=NODES.filter(n=>n.parent===nodeId).map(n=>n.id);if(relEntries.length>0){setChatFocus(relEntries);setChatResponse(`${nd.label} contains ${nd.entries||relEntries.length} entries. ${nd.gaps>0?`${nd.gaps} knowledge gap${nd.gaps>1?"s":""} need attention.`:""} ${nd.summary||""}`);}else{setChatFocus([nodeId]);setChatResponse(nd.summary||"No additional details available.");}};
    ...
    <div className={`${embedded?'':'p-4'} flex flex-col h-full min-h-0`} style={{fontFamily:"'Inter',system-ui,sans-serif"}}>
      <div className="flex items-center justify-between mb-2 flex-shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500">Engineering &middot; {NODES.filter(n=>n.type==="module").length} modules &middot; {NODES.filter(n=>n.type==="entry").length} entries &middot; {EDGES.length} relationships</p>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 flex gap-2">
        ...
        {selData&&<div className="w-2/5 bg-white border border-gray-200 rounded-lg overflow-y-auto" style={{borderLeft:`3px solid ${nodeStroke(selData)}`}}><div className="p-3"><div className="flex items-start justify-between mb-2"><div><span className="text-[10px] font-medium px-1.5 py-0.5 rounded inline-block mb-1" style={{background:nodeFill(selData),border:`1px solid ${nodeStroke(selData)}`,color:selData.hasGap?"#854d0e":selData.type==="dept"||selData.type==="system"?"#52525b":"#5b21b6"}}>{selData.type==="dept"?"Department":selData.type==="module"?"Module":selData.type==="system"?"System":selData.hasGap?"Entry (gap)":"Entry"}</span><h3 className="text-sm font-semibold text-gray-900">{selData.label}</h3></div><button onClick={()=>setSelected(null)} className="text-gray-400 hover:text-gray-600 cursor-pointer"><X className="w-4 h-4"/></button></div>
            ...
            {childEntries.length>0&&<div className="border-t border-gray-100 pt-2 mb-2"><p className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1.5">Entries</p>{childEntries.map(ce=><button key={ce.id} onClick={()=>setSelected(ce.id)} className="w-full flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-50 text-left cursor-pointer transition-colors"><span className="w-2 h-2 rounded-full flex-shrink-0" style={{backgroundColor:ce.hasGap?"#facc15":ce.status==="verified"?"#a78bfa":"#d4d4d8"}}></span><span className="text-[11px] text-gray-800 flex-1 truncate">{ce.label}</span><span className={`text-[9px] px-1.5 py-0.5 rounded ${ce.status==="verified"?"bg-violet-50 text-violet-600":"bg-gray-50 text-gray-500"}`}>{ce.status==="verified"?"Verified":"Draft"}</span></button>)}</div>}
            {selEdges.length>0&&<div className="border-t border-gray-100 pt-2 mb-2"><p className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1.5">Cross-links ({selEdges.filter(e=>e.type==="cross").length})</p>{selEdges.filter(e=>e.type==="cross").map((e,i)=>{const oid=e.from===selData.id?e.to:e.from;const o=NODES.find(n=>n.id===oid);if(!o)return null;return <button key={i} onClick={()=>{setSelected(oid);if(o.parent)setExpanded(prev=>new Set([...prev,o.parent]));}} className="w-full flex items-center gap-2 px-2 py-1.5 rounded hover:bg-violet-50 text-left cursor-pointer transition-colors"><ChevronRight className="w-3 h-3 text-gray-400"/><span className="text-[11px] text-gray-800 flex-1">{o.label}</span>{e.label&&<span className="text-[9px] text-gray-400">{e.label}</span>}</button>;})}</div>}
            <div className="border-t border-gray-100 pt-2"><button onClick={()=>onAsk(selData.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-violet-50 border border-violet-200 text-violet-700 text-[11px] font-medium hover:bg-violet-100 transition-colors cursor-pointer"><Sparkles className="w-3 h-3"/>Ask about this {selData.type}</button></div>
          </div></div>}
      </div>
