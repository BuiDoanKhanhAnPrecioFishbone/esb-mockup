...
import { CheckCircle2, Clock, AlertTriangle, ArrowRight, Plus, ChevronDown, SquareKanban, Sparkles, Layers, Pencil, Trash2, Check, X, Zap } from "lucide-react";
...
export default function SessionThanhTung({ embedded = false } = {}) {
  ...
  const addQuestion = (text, module) => { if (!text.trim()) return; setQuestions(prev => [...prev, { id: `q${++qIdCounter}`, text: text.trim(), from: "H\u00e0 Vy", fromType: "human", module }]); };
...
function HeroBar({ qCount }) {
  return <div className="rounded-lg border border-gray-200 bg-white p-4 mb-5 flex items-center gap-4"><div className="w-12 h-12 rounded-full bg-yellow-50 text-yellow-700 border border-yellow-200 text-sm font-semibold inline-flex items-center justify-center shrink-0">{BASE.initials}</div><div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-0.5 flex-wrap"><h1 className="text-lg font-semibold text-gray-900">{BASE.name}&apos;s session</h1><span className="text-[9px] px-2 py-0.5 rounded uppercase tracking-wider font-semibold border bg-blue-50 border-blue-200 text-blue-700">Prepare</span></div><p className="text-[12px] text-gray-500">{BASE.role}{" · "}{BASE.dept}</p><p className="text-[11px] text-gray-500 mt-0.5" style={{ fontFamily: "ui-monospace, Menlo, monospace" }}>{BASE.daysLeft}{"d left · "}{BASE.coworkers}{" coworkers · "}{qCount}{" Qs · "}{BASE.gaps}{" gaps"}</p></div></div>;
}

function OverviewTab({ qCount, onSwitchTab, coworkers, onAddCoworker, onRemoveCoworker }) {
  return <div className="space-y-4"><div className="rounded-lg border border-gray-200 bg-white p-5"><h3 className="text-sm font-semibold text-gray-900 mb-3">Data collection complete</h3><div className="grid grid-cols-4 gap-3 mb-4"><MC l="Boards" v={BASE.boards} /><MC l="Cards" v={BASE.cards} /><MC l="Areas" v={BASE.modules} /><MC l="Questions" v={qCount} /></div><div className="pt-3 border-t border-gray-100 space-y-2"><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium">Knowledge areas</p><div className="flex flex-wrap gap-1.5">{["Test Automation Framework", "Bug Triage & Escalation", "Release Testing", "QA Infrastructure"].map(m => <span key={m} className="text-[11px] px-2 py-1 rounded-md bg-gray-50 border border-gray-200 text-gray-700">{m}</span>)}</div></div><div className="pt-3 mt-3 border-t border-gray-100 grid grid-cols-2 gap-3"><div><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1">Coworker engagement</p><p className="text-[12px] text-gray-700">1 of 2 have joined</p></div><div><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1">Knowledge gaps</p><p className="text-[12px] text-yellow-700">{BASE.gaps}{" gaps (card-level)"}</p></div></div></div><TTCoworkerNetwork coworkers={coworkers} onAdd={onAddCoworker} onRemove={onRemoveCoworker}/><div className="flex items-center gap-3"><button onClick={onSwitchTab} className="h-9 px-4 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50 cursor-pointer">Review in Data tab</button><button className="h-9 px-4 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium inline-flex items-center gap-2 cursor-pointer transition-colors">{"Start Capture"}<ArrowRight className="w-3.5 h-3.5" /></button></div></div>;
}
...
function DataTab({ questions, onAdd, onEdit, onDelete }) {
  ...
  return <div>
    <div className="mb-4 flex items-center gap-2"><input value={generalInput} onChange={e => setGeneralInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleGeneralAsk()} placeholder="Ask a general question..." className="flex-1 h-9 px-3 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20" /><button onClick={handleGeneralAsk} className="h-9 px-3 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-xs font-medium cursor-pointer">Ask</button></div>
    {questions.filter(q => q.module === "General").length > 0 && <div className="rounded-lg border border-violet-200 bg-violet-50/20 mb-3 overflow-hidden"><div className="px-4 py-2.5 bg-violet-50/40 border-b border-violet-200 flex items-center gap-2"><Sparkles className="w-3.5 h-3.5 text-violet-500" /><span className="text-sm font-semibold text-gray-900">General questions</span><span className="text-[11px] text-gray-500">{questions.filter(q => q.module === "General").length}</span></div>{questions.filter(q => q.module === "General").map(q => <QuestionRow key={q.id} q={q} onEdit={onEdit} onDelete={onDelete} />)}</div>}
    {MODULES_DATA.map((board, bi) => <div key={bi} className="rounded-lg border border-gray-200 bg-white mb-3 overflow-hidden"><div className="px-4 py-2.5 bg-gray-50 border-b border-gray-200 flex items-center gap-2"><Layers className="w-3.5 h-3.5 text-gray-400" /><span className="text-sm font-semibold text-gray-900">{board.board}</span><span className="text-[11px] text-gray-500">{board.boardCards}c</span></div>{board.modules.map((mod, mi) => <ModuleSection key={mi} mod={mod} questions={questions.filter(q => q.module === mod.name)} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} />)}</div>)}
  </div>;
}

function QuestionRow({ q, onEdit, onDelete }) {
  const [editing, setEditing] = useState(false); const [editText, setEditText] = useState(q.text);
  const handleSave = () => { if (editText.trim()) { onEdit(q.id, editText.trim()); setEditing(false); } };
  return <div className="px-4 py-2.5 border-b border-violet-100 last:border-b-0 group">{editing?<div className="flex items-center gap-2"><input value={editText} onChange={e => setEditText(e.target.value)} onKeyDown={e => { if (e.key === "Enter") handleSave(); if (e.key === "Escape") { setEditText(q.text); setEditing(false); } }} className="flex-1 h-7 px-2 rounded border border-violet-300 text-[12px] focus:outline-none focus:ring-2 focus:ring-violet-500/20" autoFocus /><button onClick={handleSave} className="w-6 h-6 rounded bg-violet-600 text-white inline-flex items-center justify-center cursor-pointer hover:bg-violet-700"><Check className="w-3 h-3" /></button><button onClick={() => { setEditText(q.text); setEditing(false); }} className="w-6 h-6 rounded border border-gray-300 text-gray-500 inline-flex items-center justify-center cursor-pointer hover:bg-gray-50"><X className="w-3 h-3" /></button></div>:<div className="flex items-start gap-2"><div className="flex-1"><div className="text-[12px] text-gray-900">{q.text}</div><div className="text-[10px] text-gray-500 flex items-center gap-1 mt-0.5">{q.fromType === "ai" ? <Sparkles className="w-2.5 h-2.5 text-violet-500" /> : null}<span>{q.from}</span><span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 ml-1">{q.module}</span></div></div><div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"><button onClick={() => setEditing(true)} className="w-6 h-6 rounded hover:bg-gray-100 inline-flex items-center justify-center text-gray-400 hover:text-gray-600 cursor-pointer" title="Edit"><Pencil className="w-3 h-3" /></button><button onClick={() => onDelete(q.id)} className="w-6 h-6 rounded hover:bg-rose-50 inline-flex items-center justify-center text-gray-400 hover:text-rose-600 cursor-pointer" title="Delete"><Trash2 className="w-3 h-3" /></button></div></div>}</div>;
}
...
function ModuleSection({ mod, questions, onAdd, onEdit, onDelete }) {
  ...
  return <div className="border-b border-gray-100 last:border-b-0">
    <div className="w-full px-4 py-2 flex items-center justify-between hover:bg-gray-50">
      {renaming?<div className="flex items-center gap-2 flex-1" onClick={e => e.stopPropagation()}><ChevronDown className={`w-3 h-3 text-gray-400 transition-transform ${expanded ? "" : "-rotate-90"}`} /><input value={renameInput} onChange={e => setRenameInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") handleRename(); if (e.key === "Escape") { setRenameInput(displayName); setRenaming(false); } }} className="flex-1 h-7 px-2 rounded border border-violet-300 text-[13px] font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20" autoFocus /><button onClick={handleRename} className="w-6 h-6 rounded bg-violet-600 text-white inline-flex items-center justify-center cursor-pointer hover:bg-violet-700"><Check className="w-3 h-3" /></button><button onClick={() => { setRenameInput(displayName); setRenaming(false); }} className="w-6 h-6 rounded border border-gray-300 text-gray-500 inline-flex items-center justify-center cursor-pointer hover:bg-gray-50"><X className="w-3 h-3" /></button></div>:<button onClick={() => setExpanded(!expanded)} className="flex items-center gap-2 flex-1 text-left cursor-pointer"><ChevronDown className={`w-3 h-3 text-gray-400 transition-transform ${expanded ? "" : "-rotate-90"}`} /><span className="text-[13px] font-medium text-gray-900">{"Module: "}{displayName}</span><span className="text-[11px] text-gray-500">{mod.cards}c</span>{questions.length > 0 && <span className="text-[9px] px-1.5 py-0.5 rounded bg-violet-50 text-violet-600 border border-violet-200">{questions.length}Qs</span>}{totalGaps > 0 && <span className="text-[9px] px-1.5 py-0.5 rounded bg-yellow-50 text-yellow-700 border border-yellow-200">{totalGaps}{" gap"}{totalGaps > 1 ? "s" : ""}</span>}</button>}
      {!renaming && <span onClick={e => { e.stopPropagation(); setRenaming(true); setRenameInput(displayName); }} className="text-[10px] text-gray-400 hover:text-violet-600 cursor-pointer shrink-0 ml-2">Rename</span>}
    </div>
    {expanded && <>
      {mod.moduleGaps && mod.moduleGaps.length > 0 && <div className="px-4 py-2 pl-10 border-t border-gray-50"><p className="text-[10px] font-semibold text-yellow-800 mb-1.5">{displayName}{" — "}{mod.moduleGaps.length}{" gap"}{mod.moduleGaps.length > 1 ? "s" : ""}</p><div className="space-y-1">{mod.moduleGaps.map((g, gi) => <div key={gi} className="text-[10px] text-yellow-800 bg-yellow-50 border border-yellow-200 rounded-md px-2.5 py-1.5 flex items-start gap-1.5"><Sparkles className="w-3 h-3 shrink-0 mt-0.5 text-yellow-600" /><span><span className="font-semibold">{"GAP #"}{gi + 1}{": "}</span>{g}</span></div>)}</div></div>}
      {mod.items && mod.items.map((card, ci) => <div key={ci} className="w-full px-4 py-2 pl-10 flex items-center gap-2 text-left border-t border-gray-50 hover:bg-gray-50 cursor-pointer"><SquareKanban className="w-3 h-3 text-gray-400 shrink-0" /><span className="text-[12px] text-gray-800 flex-1">{card.name}</span>{card.gaps && card.gaps.length > 0 && <span className="text-[8px] px-1.5 py-0.5 rounded border bg-orange-50 text-orange-700 border-orange-400 inline-flex items-center gap-0.5"><Zap className="w-2.5 h-2.5" />detects</span>}</div>)}
      {questions.length > 0 && <div className="px-4 py-2 pl-10 border-t border-gray-100 space-y-0"><p className="text-[9px] text-violet-600 uppercase tracking-wider font-medium mb-1">{"Questions ("}{questions.length}{")"}</p>{questions.map(q => <QuestionRow key={q.id} q={q} onEdit={onEdit} onDelete={onDelete} />)}</div>}
      <div className="px-4 py-1.5 pl-10 border-t border-gray-50"><button onClick={e => { e.stopPropagation(); setShowModQ(!showModQ); }} className="text-[10px] text-violet-600 inline-flex items-center gap-1 hover:text-violet-700 cursor-pointer"><Plus className="w-2.5 h-2.5" />Ask about this module</button>{showModQ && <div className="flex gap-1.5 mt-1.5"><input value={modInput} onChange={e => setModInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleModAsk()} placeholder={`Question about ${mod.name}...`} className="flex-1 h-7 px-2 rounded border border-gray-200 text-[10px] focus:outline-none focus:ring-2 focus:ring-violet-500/20" /><button onClick={handleModAsk} className="h-7 px-2 rounded bg-violet-600 text-white text-[9px] cursor-pointer">Ask</button></div>}</div>
    </>}
  </div>;
}

function TTCoworkerNetwork({ coworkers=[], onAdd, onRemove }) {
  const [showAdd, setShowAdd] = useState(false); const [addInput, setAddInput] = useState("");
...
function LogsTab() {
  return <div className="space-y-2">{LOGS.map((log, i) => <div key={i} className="rounded-md border border-gray-200 bg-white px-3 py-2" style={{ borderLeft: `2px solid ${i === 0 ? "rgb(234,179,8)" : "rgb(229,231,235)"}`, borderRadius: 0 }}><div className="flex items-center justify-between mb-0.5 gap-2"><span className="text-[10px] text-gray-500" style={{ fontFamily: "ui-monospace, Menlo, monospace" }}>{log.time}</span><span className="text-[10px] text-gray-700 font-medium shrink-0">{log.actor}</span></div><div className="text-[11px] text-gray-900 leading-relaxed">{log.text}</div></div>)}</div>;
}
