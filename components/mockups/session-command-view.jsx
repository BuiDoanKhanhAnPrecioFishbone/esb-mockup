...
function OffboarderOverview({ stepId, onSwitchTab, onOpenQuestion }) {
  ...
  return <div className="max-w-2xl">
    <div className="relative overflow-hidden rounded-xl border border-violet-100 mb-4 px-5 py-4" style={{background:"linear-gradient(135deg, #f5f3ff 0%, #ede9fe 55%, #faf5ff 100%)"}}><h1 className="text-lg font-semibold text-gray-900 tracking-tight">{"Good afternoon, Minh Lê"}</h1><p className="text-[12px] text-gray-600 mt-0.5">{waiting.length}{" question"}{waiting.length!==1?"s":""}{" waiting for you"}</p></div>
...
function CardRow({ card, linked, showProgress, moveTargets=[], canManage, dismissedFlags, onDismissFlag, onMoveCard, primaryModuleOf, selectedCard, onSelectCard }) {
  ...
  return <div className={`group/row relative w-full flex items-center gap-2 pr-3 py-2 border-t border-gray-50 ${linked?"pl-7 border-l-2 border-l-violet-300":"pl-10"} ${isSel?"bg-violet-50 border-l-2 border-l-violet-500":"hover:bg-gray-50"}`} style={linked&&!isSel?{borderLeftStyle:"dashed"}:undefined}>
    {canManage&&<span title="Drag to move between modules" className="opacity-0 group-hover/row:opacity-100 cursor-grab text-gray-300 hover:text-gray-400 shrink-0 -ml-2"><GripVertical className="w-3 h-3"/></span>}
...
function ModuleSection({ mod, role, isCapture, isDeliver, isComplete, canEditQs, primaryCards=[], linkedCards=[], showProgress, addedQs, onAddModQ, onEditModQ, onDeleteModQ, canManage, dismissedFlags, onDismissFlag, onMoveCard, primaryModuleOf, selectedCard, onSelectCard }) {
  ...
  const [gapQs, setGapQs] = useState(moduleGapQs); const [editingGap, setEditingGap] = useState(null); const [gapEditText, setGapEditText] = useState("");
  ...
  const cardCommon = { showProgress, canManage, dismissedFlags, onDismissFlag, onMoveCard, primaryModuleOf, selectedCard, onSelectCard };
