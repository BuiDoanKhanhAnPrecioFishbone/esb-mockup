...
export const SESSION = {
  ...
  initials: "ML", daysLeft: 22, deadline: "Jun 30, 2026",
  boards: 3, cards: 64, modules: 5, questions: 14, gaps: 6, coworkers: 3,
  answered: 9, satisfied: 7, gapsAddressed: 4, files: 3,
};
const GQ = [
  { q: "What's the process for handling customer escalations?", from: "Coworker", answer: "On-call engineer first. P1: page lead via PagerDuty. Engineering provides RCA within 24h.", answeredBy: "Minh Lê", answeredAt: "1d ago", satisfiedBy: "Hà Vy", satisfiedAt: "3h" },
...
function DataContent({ role, stepId, isReady }) {
  ...
  return <div className="relative"><div>
    {!readOnly&&<div className="mb-4 flex items-center gap-2"><input placeholder="Ask a general question..." className="flex-1 h-9 px-3 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20"/><button className="h-9 px-3 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-xs font-medium">Ask</button></div>}
    {GQ.length>0&&<div className="rounded-lg border border-violet-200 bg-violet-50/20 mb-3 overflow-hidden"><div className="px-4 py-2.5 bg-violet-50/40 border-b border-violet-200 flex items-center gap-2"><MessageCircle className="w-3.5 h-3.5 text-violet-500"/><span className="text-sm font-semibold text-gray-900">General questions</span><span className="text-[11px] text-gray-500">{GQ.length}</span></div>{GQ.map((q,i)=><div key={i} className="px-4 py-2.5 border-b border-violet-100 last:border-b-0"><div className="text-[12px] text-gray-900">{q.q}</div><div className="text-[10px] text-gray-500 flex items-center gap-1 mt-0.5"><User className="w-2.5 h-2.5"/>{q.from}</div>{showAnswers&&q.answer&&<AnswerBlock q={q} role={role} committed={isComplete} readOnly={readOnly}/>}{isCapture&&!q.answer&&role==="offboarder"&&<AnswerInput/>}</div>)}</div>}
    {MODULES_DATA.map((board,bi)=><div key={bi} className="rounded-lg border border-gray-200 bg-white mb-3 overflow-hidden"><div className="px-4 py-2.5 bg-gray-50 border-b border-gray-200 flex items-center gap-2"><Layers className="w-3.5 h-3.5 text-gray-400"/><span className="text-sm font-semibold text-gray-900">{board.board}</span><span className="text-[11px] text-gray-500">{board.boardCards}c</span></div>{board.modules.map((mod,mi)=><ModuleSection key={mi} mod={mod} role={role} isCapture={isCapture} isDeliver={isDeliver} isComplete={isComplete} selectedCard={selectedCard} onSelectCard={setSelectedCard}/>)}</div>)}
...
function ModuleSection({ mod, role, isCapture, isDeliver, isComplete, selectedCard, onSelectCard }) {
  const [expanded, setExpanded] = useState(true); const [showModQ, setShowModQ] = useState(false);
  ...
  return <div className="border-b border-gray-100 last:border-b-0">
    <button onClick={()=>setExpanded(!expanded)} className="w-full px-4 py-2 flex items-center justify-between text-left hover:bg-gray-50"><div className="flex items-center gap-2"><ChevronDown className={`w-3 h-3 text-gray-400 transition-transform ${expanded?"":"-rotate-90"}`}/><span className="text-[13px] font-medium text-gray-900">{mod.name}</span><span className="text-[11px] text-gray-500">{mod.cards}c</span>{mod.qs>0&&<span className="text-[9px] px-1.5 py-0.5 rounded bg-violet-50 text-violet-600 border border-violet-200">{mod.qs}Qs</span>}{totalGaps>0&&<span className="text-[9px] px-1.5 py-0.5 rounded bg-yellow-50 text-yellow-700 border border-yellow-200">{totalGaps}gap{totalGaps>1?"s":""}</span>}{showProgress&&prog.total>0&&<span className={`text-[9px] px-1.5 py-0.5 rounded border ${prog.answered===prog.total?"bg-emerald-50 text-emerald-700 border-emerald-200":"bg-gray-50 text-gray-600 border-gray-200"}`}>{prog.answered}/{prog.total}</span>}</div>{role==="manager"&&!readOnly&&<span className="flex items-center gap-2 text-[10px] text-gray-400"><span className="hover:text-violet-600 inline-flex items-center gap-0.5"><Upload className="w-2.5 h-2.5"/>Upload</span><span className="hover:text-violet-600">Rename</span></span>}</button>
    {expanded&&<>
...
function SidePanel({ card, role, onClose, isCapture, isDeliver, isComplete }) {
  ...
  return <div className="p-5">
    ...
    {role!=="offboarder"&&!readOnly&&<button className="text-[10px] text-violet-600 inline-flex items-center gap-1 mt-1 hover:text-violet-700"><Upload className="w-2.5 h-2.5"/>Upload file</button>}
...
function LogsContent({ role, stepId }) {
  ...
  return <div className="space-y-1.5"><div className="flex gap-2 mb-3">{["All","System","Questions","Files","Edits"].map(f=><button key={f} className={`px-2.5 py-1 rounded-md text-[11px] ${f==="All"?"bg-violet-50 text-violet-700 font-medium":"text-gray-500 hover:bg-gray-100"}`}>{f}</button>)}</div>{logs.map((l,i)=>{const border=l.accent||(l.type==="system"?"rgb(229,231,235)":l.type==="questions"?"rgb(124,58,237)":"rgb(234,179,8)");return <div key={i} className="flex items-start gap-3 px-3 py-2 rounded-md border border-gray-200 bg-white" style={{borderLeft:`2px solid ${border}`}}><span className="text-[10px] text-gray-500 shrink-0 mt-0.5" style={{fontFamily:"ui-monospace,Menlo,monospace"}}>{l.ts}</span><span className={`text-[9px] px-1.5 py-0.5 rounded uppercase tracking-wider font-medium shrink-0 ${l.type==="questions"?"bg-violet-50 text-violet-600":l.type==="files"?"bg-yellow-50 text-yellow-700":"bg-gray-100 text-gray-500"}`}>{l.type}</span><span className="text-[11px] text-gray-900">{l.text}</span></div>})}</div>;
}
