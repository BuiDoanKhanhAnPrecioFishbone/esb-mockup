...
function AIReasoningPanel({ card, onClose, readOnly = false }) {
  ...
  const [assign, setAssign] = useState(cls.primary ? [cls.primary, ...(cls.linked||[])] : []);
  const [adding, setAdding] = useState(false);
  const [showAssign, setShowAssign] = useState(cls.state==="review"||cls.state==="uncat"); // RF-01/02 — multi-select visible
  const [accepting, setAccepting] = useState(false); // RF-01 — New Module inline name edit
  const [newName, setNewName] = useState(cls.newModule||"");
  const [done, setDone] = useState(null);
  const addable = ALL_MOD_NAMES.filter(m=>!assign.includes(m));
  const removeM = (m)=>setAssign(a=>a.filter(x=>x!==m));
  const confirmAssign = () => setDone(assign.length?`Assigned to ${assign.join(", ")}.`:"Left uncategorized.");
  const verdictBox = cls.state==="pass"
    ? <div className="rounded-md bg-emerald-50 border border-emerald-200 px-3 py-2 text-[11px] text-emerald-800 flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5"/>{assign.length>1?`Assigned to ${assign.join(", ")}`:"No action needed"}</div>
    : cls.state==="review" ? <div className="rounded-md bg-amber-50 border border-amber-200 px-3 py-2 text-[11px] text-amber-800 flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5"/>Needs your call — pick the module(s).</div>
  ...
  // RF-03 — shared multi-select: all chips equal-weight (× only, no ★/↗). Used by Review, Uncategorized, New-Module-Skip, and Change-assignment.
  const multiSelect = <><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1.5">Assign to</p>
    <div className="flex flex-wrap gap-1.5 mb-2">{assign.map(m=><span key={m} className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-violet-50 text-violet-700 border border-violet-200">{m}<button onClick={()=>removeM(m)} className="hover:text-rose-500 cursor-pointer">×</button></span>)}
      <div className="relative"><button onClick={()=>setAdding(a=>!a)} className="text-[10px] px-2 py-0.5 rounded-full border border-gray-200 text-gray-500 hover:border-violet-200 hover:text-violet-600 cursor-pointer inline-flex items-center gap-0.5"><Plus className="w-2.5 h-2.5"/>Add module</button>{adding&&<div className="absolute left-0 top-full mt-1 z-10 w-44 rounded-md border border-gray-200 bg-white shadow-lg py-1 max-h-44 overflow-y-auto">{addable.length?addable.map(m=><button key={m} onClick={()=>{setAssign(a=>[...a,m]);setAdding(false);}} className="w-full text-left px-2.5 py-1 text-[11px] text-gray-700 hover:bg-violet-50 hover:text-violet-700 cursor-pointer">{m}</button>):<p className="px-2.5 py-1 text-[10px] text-gray-400">All modules added</p>}</div>}</div>
    </div>
    <button onClick={confirmAssign} className="h-7 px-3 rounded-md text-[11px] font-medium inline-flex items-center gap-1.5 bg-violet-600 text-white hover:bg-violet-700 cursor-pointer">{cls.state==="uncat"?"Assign":"Confirm"}</button></>;
  return <div className="fixed top-0 right-[480px] h-full w-[400px] bg-white border-l border-gray-200 shadow-xl z-50 overflow-y-auto" onClick={e=>e.stopPropagation()}>
    <div className="p-4">
      ...
      {readOnly ? <p className="text-[10px] text-gray-400 flex items-center gap-1.5"><User className="w-3 h-3"/>Read-only — only the Manager can change the assignment.</p>
        ...
        : accepting ? <div><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1.5">Name the new module</p><div className="flex items-center gap-1.5"><input value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")setDone(`Created module “${(newName.trim()||cls.newModule)}” and assigned this card.`);if(e.key==="Escape")setAccepting(false);}} placeholder="Module name" className="flex-1 h-7 px-2 rounded border border-violet-300 text-[11px] focus:outline-none focus:ring-2 focus:ring-violet-500/20" autoFocus/><button onClick={()=>setDone(`Created module “${(newName.trim()||cls.newModule)}” and assigned this card.`)} className="w-7 h-7 rounded bg-violet-600 text-white inline-flex items-center justify-center cursor-pointer hover:bg-violet-700"><Check className="w-3.5 h-3.5"/></button><button onClick={()=>setAccepting(false)} className="w-7 h-7 rounded border border-gray-300 text-gray-500 inline-flex items-center justify-center cursor-pointer hover:bg-gray-50"><X className="w-3.5 h-3.5"/></button></div></div>
        : cls.state==="newmod"&&!showAssign ? <div className="flex flex-wrap gap-2"><button onClick={()=>{setAccepting(true);setNewName(cls.newModule);}} className="h-7 px-3 rounded-md bg-violet-600 text-white text-[11px] font-medium hover:bg-violet-700 cursor-pointer">Accept</button><button onClick={()=>{setShowAssign(true);setAssign([]);}} className="h-7 px-3 rounded-md border border-gray-300 text-gray-600 text-[11px] font-medium hover:bg-gray-50 cursor-pointer">Skip</button></div>
        : (cls.state==="pass"&&!showAssign) ? <button onClick={()=>setShowAssign(true)} className="text-[11px] text-violet-600 hover:text-violet-700 cursor-pointer inline-flex items-center gap-1"><Layers className="w-3 h-3"/>Change assignment</button>
        : multiSelect}
    </div>
  </div>;
}
...
function CardRow({ card, linked, showProgress, canManage, dismissedFlags, onDismissFlag, primaryModuleOf, selectedCard, onSelectCard, clsFilter }) {
  ...
  if (clsFilter && clsFilter!=="all" && cls.state!==clsFilter) return null;
  // §4.2 — no drag/Move-to/attachment. §4.1 — orange Detects. §4.6 — Review/New Module/Uncategorized badge + left accent (Pass is quiet).
  return <div className={`group/row relative w-full flex items-center gap-2 pr-3 py-2 border-t border-gray-50 ${linked?"pl-7 border-l-2 border-l-violet-300":"pl-10"} ${isSel?"bg-violet-50 border-l-2 border-l-violet-500":"hover:bg-gray-50"}`} style={!isSel&&!linked&&cmeta.border?{borderLeft:`2px solid ${cmeta.border}`}:(linked&&!isSel?{borderLeftStyle:"dashed"}:undefined)}>
    ...
    {cls.state!=="pass"&&<span className={`text-[8px] px-1.5 py-0.5 rounded border inline-flex items-center gap-0.5 shrink-0 ${cmeta.badge}`} title={`AI: ${cmeta.label}`}>{cls.state==="review"?<AlertTriangle className="w-2.5 h-2.5"/>:cls.state==="newmod"?<Sparkles className="w-2.5 h-2.5"/>:<Inbox className="w-2.5 h-2.5"/>}{cmeta.label}</span>}
...
function SidePanel({ card, role, onClose, isCapture, isDeliver, isComplete, primaryModule, sat, onOpenReasoning }) {
  ...
  return <div className="p-5">
    ...
    {/* §4.8 — Module + Classification merged into one chip; click the › chip to open AI reasoning. */}
    <p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mt-1 mb-1.5">Module &middot; Classification</p>
    <div className="flex flex-wrap gap-1.5 mb-1">{(()=>{const cls=classify(card);const open=onOpenReasoning?()=>onOpenReasoning(card):undefined;
      ...
      return <><button onClick={open} className="text-[10px] px-2 py-0.5 rounded border border-violet-200 bg-violet-50 text-violet-700 font-medium inline-flex items-center gap-1 cursor-pointer">{primaryModule}{open&&<ChevronRight className="w-2.5 h-2.5 text-violet-400"/>}</button>{linkedMods.map(m=><span key={m} className="text-[10px] px-2 py-0.5 rounded bg-violet-50 text-violet-700 border border-violet-200">{m}</span>)}</>;
    })()}</div>
