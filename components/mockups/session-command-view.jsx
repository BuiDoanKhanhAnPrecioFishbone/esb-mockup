...
/* Session Command View — CL-119/120/127/128/129
   ...
   Session-level upload: all roles, Prepare + Capture.
   Module headers: Rename only (no Upload).
   Paperclip attach on all question inputs. */
...
function DataContent({ role, stepId, isReady, canEditQs, generalQs, addedModQs, onAddGQ, onEditGQ, onDeleteGQ, onAddModQ, onEditModQ, onDeleteModQ, focusQ, focusKey, uploads, onAddUpload, onDeleteUpload }) {
  ...
  return <div className="relative"><div>
    {!readOnly&&<UploadableInputBar gqInput={gqInput} setGqInput={setGqInput} handleGQAsk={handleGQAsk} uploads={uploads} onAddUpload={onAddUpload} onDeleteUpload={onDeleteUpload}/>}
...
function ModuleSection({ mod, role, isCapture, isDeliver, isComplete, canEditQs, selectedCard, onSelectCard, addedQs, onAddModQ, onEditModQ, onDeleteModQ }) {
  ...
  return <div className="border-b border-gray-100 last:border-b-0">
    <button onClick={()=>setExpanded(!expanded)} className="w-full px-4 py-2 flex items-center justify-between text-left hover:bg-gray-50 cursor-pointer"><div className="flex items-center gap-2"><ChevronDown className={`w-3 h-3 text-gray-400 transition-transform ${expanded?"":"-rotate-90"}`}/><span className="text-[13px] font-medium text-gray-900">{mod.name}</span><span className="text-[11px] text-gray-500">{mod.cards}c</span>{totalQs>0&&<span className="text-[9px] px-1.5 py-0.5 rounded bg-violet-50 text-violet-600 border border-violet-200">{totalQs}Qs</span>}{totalGaps>0&&<span className="text-[9px] px-1.5 py-0.5 rounded bg-yellow-50 text-yellow-700 border border-yellow-200">{totalGaps}{"gap"}{totalGaps>1?"s":""}</span>}{showProgress&&prog.total>0&&<span className={`text-[9px] px-1.5 py-0.5 rounded border ${prog.answered===prog.total?"bg-emerald-50 text-emerald-700 border-emerald-200":"bg-gray-50 text-gray-600 border-gray-200"}`}>{prog.answered}/{prog.total}</span>}</div>{role==="manager"&&!readOnly&&<span className="text-[10px] text-gray-400 hover:text-violet-600 cursor-pointer">Rename</span>}</button>
    {expanded&&<>
...
function UploadableInputBar({ gqInput, setGqInput, handleGQAsk, uploads, onAddUpload, onDeleteUpload }) {
  ...
  return <div className="mb-4">
    <div className="flex items-center gap-2 mb-2">
      <input value={gqInput} onChange={e=>setGqInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleGQAsk()} placeholder="Ask a general question..." className="flex-1 h-9 px-3 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20"/>
      ...
      <button onClick={handleGQAsk} className="h-9 px-3 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-xs font-medium cursor-pointer">Ask</button>
...
function SidePanel({ card, role, onClose, isCapture, isDeliver, isComplete }) {
  ...
  return <div className="p-5">
    ...
    {role!=="offboarder"&&!readOnly&&<><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mt-3 mb-1 pt-2 border-t border-gray-100">Ask a question</p><div className="flex gap-1.5"><input value={followUp} onChange={e=>setFollowUp(e.target.value)} placeholder="Your question..." className="flex-1 h-8 px-2.5 rounded-md border border-gray-200 text-[11px] focus:outline-none focus:ring-2 focus:ring-violet-500/20"/><button className="h-8 w-8 rounded-md border border-gray-200 text-gray-400 hover:text-violet-600 inline-flex items-center justify-center cursor-pointer" title="Attach file"><Paperclip className="w-3 h-3"/></button><button className="h-8 px-2.5 rounded-md bg-violet-600 text-white text-[10px] font-medium cursor-pointer">Ask</button></div></>}
  </div>;
}
