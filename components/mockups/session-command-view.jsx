...
import { ChevronLeft, ChevronRight, ArrowRight, ArrowUpRight, X, CheckCircle2, Clock, AlertTriangle, Sparkles, Bell, Layers, User, FileText, ChevronDown, Plus, MessageCircle, Paperclip, Pencil, Trash2, Check, GripVertical, HelpCircle, Inbox, ExternalLink, Mic, Pause, SkipForward, RotateCcw, Zap, MoreHorizontal, Circle, Package, MessageSquare, Database, SquareKanban } from "lucide-react";
import { DeliverOverview, CompleteOverview } from "./session-deliver";
import { useViewAs } from "@/lib/view-as";
import { tabVisibility } from "@/lib/view-matrix";
import { SESSION, MODULES_DATA, UNCATEGORIZED, CLASSIFY, OB_QUEUE, SEED_COWORKERS, SEED_GQ, OFFBOARDER_QUEUE } from "@/lib/data";
...
// Offboarder question queue (OQ-01..11) — the Offboarder's single-purpose Capture
// surface. All context is inline (no side panel, no "See in context"); the Data tab is
// locked. Derives from OFFBOARDER_QUEUE; general questions are counted alongside cards.
function OffboarderWorkspace() {
  const [answers, setAnswers] = useState(()=>{ const m={}; OFFBOARDER_QUEUE.forEach(it=>{ if(it.answered) m[it.id]={ text:it.answer, status: it.revision?"revision":it.accepted?"accepted":"waiting", edited:false }; }); return m; });
  const [drafts, setDrafts] = useState({});
  const [editing, setEditing] = useState(null); const [editText, setEditText] = useState("");
  const [filter, setFilter] = useState("all");
  const total = OFFBOARDER_QUEUE.length;
  const answeredCount = Object.keys(answers).length;
  const daysLeft = SESSION.daysLeft;
  const submit = (id) => { const t=(drafts[id]||"").trim(); if(!t) return; setAnswers(p=>({...p,[id]:{text:t,status:"waiting",edited:false}})); setDrafts(p=>({...p,[id]:""})); };
  const resubmit = (id) => { const t=(drafts[id]||"").trim(); if(!t) return; setAnswers(p=>({...p,[id]:{text:t,status:"waiting",edited:true}})); setDrafts(p=>({...p,[id]:""})); };
  const saveEdit = (id) => { if(editText.trim()) setAnswers(p=>({...p,[id]:{...p[id],text:editText.trim(),edited:true}})); setEditing(null); };
  // OQ-02 — module filter chips (counts from the full queue).
  const modOrder=[]; OFFBOARDER_QUEUE.forEach(it=>{ if(!modOrder.includes(it.module)) modOrder.push(it.module); });
  const chips=[{key:"all",label:"All",count:total},...modOrder.map(mo=>({key:mo,label:mo,count:OFFBOARDER_QUEUE.filter(it=>it.module===mo).length}))];
  const inFilter=(it)=> filter==="all"||it.module===filter;
  const stOf=(it)=> answers[it.id]?.status;
  const filtered = OFFBOARDER_QUEUE.filter(inFilter);
  // OQ-04 — priority sort: revision, then waiting (gap > card > general), then answered.
  const revisionItems = filtered.filter(it=>stOf(it)==="revision");
  const rank={ gap:0, card:1, general:2 };
  const waitingItems = filtered.filter(it=>!answers[it.id]).sort((a,b)=>rank[a.type]-rank[b.type]);
  const answeredItems = filtered.filter(it=>answers[it.id]&&stOf(it)!=="revision");
  const allDone = waitingItems.length===0 && revisionItems.length===0;
  const AV_TONE={manager:"bg-violet-100 text-violet-700",coworker:"bg-teal-100 text-teal-700",ai:"bg-gray-100 text-gray-500"};
  const Av=({it})=><span className={`w-4 h-4 rounded text-[8px] font-semibold inline-flex items-center justify-center shrink-0 ${AV_TONE[it.tone]||AV_TONE.ai}`}>{it.avatar}</span>;
  const Attr=({it})=><div className="flex items-center gap-1.5 text-[11px] text-gray-500"><Av it={it}/><span>{it.from}</span><span className="text-gray-300">·</span><span className="px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 text-[10px]">{it.type==="gap"?`Gap · ${it.module}`:it.module}</span></div>;
  const CardCtx=({c})=><div className="rounded-md bg-gray-50 border border-gray-200 px-3 py-2 text-[11px] mt-2"><div className="flex items-center gap-1.5 text-gray-800 font-medium"><SquareKanban className="w-3 h-3 text-gray-400 shrink-0"/>{c.cardName}</div>{c.alsoIn.length>0&&<div className="text-[10px] text-indigo-500 mt-0.5 pl-4">{"Also in: "}{c.alsoIn.join(", ")}</div>}<p className="text-gray-600 mt-1">{c.desc}</p>{(c.files.length>0||c.total>0)&&<div className="text-[10px] text-gray-400 mt-1 flex items-center gap-2 flex-wrap">{c.files.map(f=><span key={f.name} className="inline-flex items-center gap-0.5"><Paperclip className="w-2.5 h-2.5"/>{f.name}</span>)}{c.total>0&&<span>{"Checklist: "}{c.done}{"/"}{c.total}</span>}</div>}</div>;
  const GapCtx=({c})=><div className="rounded-md bg-yellow-50 border border-yellow-200 px-3 py-2 text-[11px] mt-2"><div className="flex items-start gap-1.5 text-yellow-800 font-medium"><AlertTriangle className="w-3 h-3 mt-0.5 shrink-0 text-yellow-600"/>{c.title}</div><p className="text-yellow-700 mt-1">{c.summary}</p>{c.related.length>0&&<p className="text-[10px] text-yellow-700 opacity-80 mt-1">{"Related: "}{c.related.join(" · ")}</p>}</div>;
  const Submit=({id,label,onClick})=>{const empty=!(drafts[id]||"").trim();return <button disabled={empty} onClick={onClick} className={`h-7 px-3 rounded-md text-[11px] font-medium ${empty?"bg-gray-100 text-gray-400 cursor-not-allowed":"bg-violet-600 text-white hover:bg-violet-700 cursor-pointer"}`}>{label}</button>;};
  const borderOf=(t)=>t==="card"?"border-l-violet-400":t==="gap"?"border-l-amber-400":"border-l-gray-300";
  return <div className="max-w-2xl">
    <div className="flex items-start justify-between gap-4 mb-4">
      <div><h2 className="text-base font-semibold text-gray-900">Your questions</h2><p className="text-[12px] text-gray-500 mt-0.5">Answer these to preserve your knowledge for the team.</p></div>
      <div className="text-right shrink-0 w-[150px]"><div className="text-[12px] font-semibold text-gray-900">{answeredCount}{" of "}{total}{" answered"}</div><div className="h-[5px] rounded-full bg-gray-200 overflow-hidden my-1"><div className="h-full rounded-full" style={{width:`${Math.round(answeredCount/total*100)}%`,background:"linear-gradient(90deg,#8b5cf6,#7c3aed)"}}/></div><div className={`text-[10px] ${daysLeft<7?"text-rose-600":daysLeft<30?"text-amber-600":"text-gray-500"}`} style={{fontFamily:"ui-monospace,Menlo,monospace"}}>{daysLeft}{" days left"}</div></div>
    </div>
    <div className="flex flex-wrap gap-1.5 mb-4">{chips.map(ch=><button key={ch.key} onClick={()=>setFilter(ch.key)} className={`text-[11px] px-2.5 py-1 rounded-md border cursor-pointer ${filter===ch.key?"bg-violet-50 border-violet-300 text-violet-700":"border-gray-200 text-gray-500 hover:border-gray-300"}`}>{ch.label}{" "}<span className="opacity-60" style={{fontFamily:"ui-monospace,Menlo,monospace"}}>{ch.count}</span></button>)}</div>
    {allDone&&<div className="rounded-xl border border-emerald-200 p-6 text-center" style={{background:"linear-gradient(135deg,#f0fdf4,#dcfce7)"}}><CheckCircle2 className="w-8 h-8 text-emerald-500 mx-auto mb-2"/><h3 className="text-sm font-medium text-emerald-700">All questions answered</h3><p className="text-[11px] text-gray-500 mt-1">{"Hà Vy will review your answers before committing to the Knowledge Graph."}</p></div>}
    {revisionItems.length>0&&<div className="mb-5"><p className="text-[11px] font-semibold text-amber-700 mb-2">{"Needs revision · "}{revisionItems.length}</p><div className="space-y-2.5">{revisionItems.map(it=><div key={it.id} className={`rounded-lg border border-amber-200 bg-white border-l-[3px] ${borderOf("card")} px-4 py-3`}><p className="text-[13px] text-gray-900 mb-1.5">{it.q}</p><Attr it={it}/><div className="rounded-md bg-gray-50 px-3 py-2 mt-2"><p className="text-[11px] text-gray-400 line-through">{answers[it.id]?.text}</p></div><div className="rounded-md bg-yellow-50 border border-yellow-200 px-3 py-2 text-[11px] mt-2"><div className="flex items-center gap-1.5 mb-1 text-yellow-800"><span className="w-4 h-4 rounded bg-violet-100 text-violet-700 text-[8px] font-semibold inline-flex items-center justify-center">HV</span><span className="font-medium">{it.revision.by}{" requested a revision:"}</span></div><p className="italic text-yellow-800">{'"'}{it.revision.note}{'"'}</p></div><textarea value={drafts[it.id]||""} onChange={e=>setDrafts(p=>({...p,[it.id]:e.target.value}))} placeholder="Revise your answer..." className="w-full h-16 px-2 py-1.5 mt-2 rounded border border-yellow-300 bg-yellow-50/40 text-[11px] resize-none focus:outline-none focus:ring-2 focus:ring-yellow-500/20"/><div className="flex justify-end mt-1.5"><Submit id={it.id} label="Resubmit" onClick={()=>resubmit(it.id)}/></div></div>)}</div></div>}
    {waitingItems.length>0&&<div className="mb-5"><p className="text-[11px] font-semibold text-gray-700 mb-2">{"Waiting for you · "}{waitingItems.length}</p><div className="space-y-2.5">{waitingItems.map(it=><div key={it.id} className={`rounded-lg border border-gray-200 bg-white border-l-[3px] ${borderOf(it.type)} px-4 py-3`}><p className="text-[13px] text-gray-900 mb-1.5">{it.q}</p><Attr it={it}/>{it.type==="card"&&<CardCtx c={it.ctx}/>}{it.type==="gap"&&<GapCtx c={it.ctx}/>}<div className="mt-2.5"><textarea value={drafts[it.id]||""} onChange={e=>setDrafts(p=>({...p,[it.id]:e.target.value}))} placeholder="Type your answer..." className="w-full h-16 px-2 py-1.5 rounded border border-gray-200 text-[11px] resize-none focus:outline-none focus:ring-2 focus:ring-violet-500/20"/><div className="flex justify-end mt-1.5"><Submit id={it.id} label="Submit" onClick={()=>submit(it.id)}/></div></div></div>)}</div></div>}
    {answeredItems.length>0&&<div><p className="text-[11px] font-semibold text-gray-700 mb-2">{"Answered · "}{answeredItems.length}</p><div className="space-y-2.5">{answeredItems.map(it=>{const a=answers[it.id];return <div key={it.id} className="group rounded-lg border border-gray-200 bg-white px-4 py-3"><p className="text-[13px] text-gray-900 mb-1.5">{it.q}</p><Attr it={it}/>{editing===it.id?<div className="mt-2"><textarea value={editText} onChange={e=>setEditText(e.target.value)} className="w-full h-16 px-2 py-1.5 rounded border border-violet-300 text-[11px] resize-none focus:outline-none focus:ring-2 focus:ring-violet-500/20" autoFocus/><div className="flex justify-end gap-1.5 mt-1.5"><button onClick={()=>saveEdit(it.id)} className="h-7 px-3 rounded-md bg-violet-600 text-white text-[11px] font-medium hover:bg-violet-700 cursor-pointer">Save</button><button onClick={()=>setEditing(null)} className="h-7 px-3 rounded-md border border-gray-300 text-gray-600 text-[11px] font-medium hover:bg-gray-50 cursor-pointer">Cancel</button></div></div>:<div className="mt-2 rounded-md bg-gray-50 border-l-2 border-emerald-400 px-3 py-2 relative"><p className="text-[11px] text-gray-800 leading-relaxed pr-6">{a?.text}</p><button onClick={()=>{setEditing(it.id);setEditText(a?.text||"");}} className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 w-6 h-6 rounded border border-gray-200 bg-white hover:bg-gray-100 inline-flex items-center justify-center text-gray-400 hover:text-violet-600 cursor-pointer" title="Edit answer"><Pencil className="w-3 h-3"/></button></div>}<div className="mt-1.5">{a?.status==="accepted"?<span className="text-[9px] px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200 inline-flex items-center gap-0.5"><CheckCircle2 className="w-2.5 h-2.5"/>Accepted</span>:<span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 border border-gray-200">Waiting review</span>}{a?.edited&&<span className="text-[9px] px-1.5 py-0.5 rounded bg-violet-50 text-violet-600 border border-violet-200 ml-1">Edited</span>}</div></div>;})}</div></div>}
  </div>;
}
...
function AIReasoningPanel({ card, onClose, readOnly = false }) {
  ...
  // MC-08 — the verdict/alert box is gone entirely; the AI reasoning conversation below carries the explanation.
  return <div className="fixed top-0 right-[480px] h-full w-[400px] bg-white border-l border-gray-200 shadow-xl z-50 flex flex-col" onClick={e=>e.stopPropagation()}>
    <div className="px-4 pt-4 pb-2 shrink-0">
      <div className="flex items-center justify-between mb-2"><div className="flex items-center gap-1.5"><Sparkles className="w-4 h-4 text-violet-600"/><h3 className="text-[14px] font-semibold text-gray-900">Module Classification</h3>{readOnly&&<span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 border border-gray-200">Read-only</span>}</div><button onClick={onClose} className="w-7 h-7 rounded-md hover:bg-gray-100 inline-flex items-center justify-center text-gray-400 cursor-pointer"><X className="w-4 h-4"/></button></div>
      <p className="text-[11px] text-gray-500">{card.name}</p>
    </div>
    {/* MC-09 — AI reasoning + confidence scroll here; the Assigned-modules block is pinned in the footer below. */}
    <div className="flex-1 overflow-y-auto px-4 pb-4">
      {/* MC-04 — AI conversation ALWAYS visible. MC-10 — no module-name suffix on the heading. */}
      <p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1.5">AI reasoning</p>
      <div className="space-y-2 mb-4">{(cls.chat||[]).map((m,i)=>{const av=AGENT_AV[m.a];return <div key={i} className={`flex gap-2 ${m.a==="G"?"pl-5":""}`}><div className="w-5 h-5 rounded-full inline-flex items-center justify-center text-[9px] font-semibold shrink-0 mt-0.5" style={{background:av.bg,color:av.fg}}>{m.a}</div><div className="flex-1 rounded-lg px-2.5 py-1.5" style={{background:m.a==="M"?"#faf5ff":"#fff7ed",borderLeft:`2px solid ${av.fg}`}}><p className="text-[8px] uppercase tracking-wider font-semibold mb-0.5" style={{color:av.fg}}>{m.step}</p><p className="text-[10px] text-gray-700 leading-snug">{m.t}</p></div></div>;})}</div>
      {/* Confidence — MC-10: no module-name suffix. */}
      <div><div className="flex items-center justify-between text-[10px] mb-1"><span className="text-gray-500 uppercase tracking-wider font-medium">Confidence</span><span style={{fontFamily:"ui-monospace,Menlo,monospace",color:confColor(cls.confidence)}}>{cls.confidence}%</span></div><div className="h-1.5 rounded-full bg-gray-100 overflow-hidden"><div className="h-full rounded-full" style={{width:`${cls.confidence}%`,background:confColor(cls.confidence)}}/></div></div>
    </div>
    {/* MC-09 — pinned footer: assigned modules + Save/Cancel (or Accept/Skip), always visible regardless of scroll. */}
    <div className="shrink-0 border-t border-gray-200 bg-white px-4 py-3">
      ...
      {/* MC-05 — Save/Cancel (or New-Module Accept/Skip), pinned with the chips they act on. */}
      {readOnly ? <p className="text-[10px] text-gray-400 flex items-center gap-1.5 mt-2"><User className="w-3 h-3"/>Read-only — only the Manager can change the assignment.</p>
        : saved ? <p className="text-[11px] text-emerald-600 flex items-center gap-1.5 mt-2"><CheckCircle2 className="w-3.5 h-3.5"/>{saved}</p>
        : accepting ? null
        : isNew ? <div className="flex flex-wrap gap-2 mt-2"><button onClick={()=>{setAccepting(true);setNewName(cls.newModule);}} className="h-7 px-3 rounded-md bg-violet-600 text-white text-[11px] font-medium hover:bg-violet-700 cursor-pointer">Accept</button><button onClick={()=>{setSkipped(true);setAssign([]);setActive(null);}} className="h-7 px-3 rounded-md border border-gray-300 text-gray-600 text-[11px] font-medium hover:bg-gray-50 cursor-pointer">Skip</button></div>
        : showSave ? <div className="flex gap-2 mt-2"><button onClick={doSave} className="h-7 px-3 rounded-md bg-violet-600 text-white text-[11px] font-medium hover:bg-violet-700 cursor-pointer">Save</button><button onClick={doCancel} className="h-7 px-3 rounded-md border border-gray-300 text-gray-600 text-[11px] font-medium hover:bg-gray-50 cursor-pointer">Cancel</button></div>
        : null}
    </div>
  </div>;
}
...
function CardRow({ card, linked, showProgress, canManage, dismissedFlags, onDismissFlag, onRestoreFlag, primaryModuleOf, selectedCard, onSelectCard, clsFilter, clsOn, moduleName }) {
  ...
  return <div className={`group/row relative w-full flex items-start gap-2 pr-3 py-2 pl-10 border-t border-gray-50 ${isSel?"bg-violet-50 border-l-2 border-l-violet-500":showSignal?cmeta.rowBg:"hover:bg-gray-50"}`} style={showSignal?{borderLeft:cmeta.rowBorder}:undefined}>
    ...
    <button onClick={()=>onSelectCard(card)} className="flex items-start gap-2 flex-1 min-w-0 text-left cursor-pointer"><SquareKanban className={`w-3 h-3 shrink-0 mt-0.5 ${linked?"text-violet-400":"text-gray-400"}`}/><div className="min-w-0"><div className="flex items-center gap-1.5"><span className={`text-[12px] truncate min-w-0 ${linked?"text-gray-500":"text-gray-800"}`}>{card.name}</span>{clsOn&&cls.state!=="pass"&&<span className={`text-[11px] px-2 py-0.5 rounded border inline-flex items-center gap-1 shrink-0 ${cmeta.badge}`} title={`AI: ${cmeta.label}`}>{cls.state==="review"?<AlertTriangle className="w-3 h-3"/>:cls.state==="newmod"?<Sparkles className="w-3 h-3"/>:<Inbox className="w-3 h-3"/>}{cmeta.label}</span>}</div>{alsoIn.length>0&&<span className="text-[10px] text-indigo-500 block truncate">{"Also in: "}{alsoIn.join(", ")}</span>}</div></button>
