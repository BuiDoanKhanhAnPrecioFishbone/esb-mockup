...
import {
  ...
  ChevronDown, Plus, Search, MessageCircle, Upload, Paperclip
} from "lucide-react";

/* ═══ Session Command View — CL-119/120/127 + all fixes + module gaps ═══
   Prepare phase · 3 roles · Right drawer Side Panel
   Gaps: card-level (metadata) + module-level (AI cross-card) */
...
const FLOW = [
  ...
  { id: "ready", label: "Ready for review", trigger: "Crawl done. 5 modules, 14 questions, 6 gaps." },
...
const SESSION = {
  ...
  boards: 3, cards: 64, modules: 5, questions: 14, gaps: 6, coworkers: 3,
};

const GENERAL_QUESTIONS = [
  { q: "What's the process for handling customer escalations?", from: "Coworker" },
  { q: "Are there any undocumented vendor agreements I should know about?", from: "Hà Vy" },
];

const MODULES_DATA = [
  { board: "Backend Services", boardCards: 34, modules: [
    { name: "Payment Service", cards: 12, qs: 4, gaps: 1, moduleGaps: ["No cards cover disaster recovery or failover procedures for payment processing"], items: [
      { name: "Kafka retry configuration", desc: "Configures retry behavior for Kafka consumers when processing fails. Includes dead letter queue routing, backoff strategy, and poison message handling. Current config uses exponential backoff with max 5 retries.", checklist: [{ text: "DLQ routing configured", done: true }, { text: "Backoff strategy documented", done: false }, { text: "Alert on repeated failures", done: false }], gaps: ["Incomplete checklist — 2 of 3 items not done"], qs: [{ q: "How does the retry logic handle poison messages?", from: "AI-generated" }, { q: "What's the max retry count before DLQ routing?", from: "AI-generated" }], files: [{ name: "kafka-config.yaml", size: "3.2 KB" }] },
      { name: "Payment gateway timeout handling", desc: "Timeout configuration and fallback behavior for payment gateway calls. Uses circuit breaker pattern with 30s timeout.", checklist: [], gaps: [], files: [], qs: [{ q: "What happens when the gateway times out mid-transaction?", from: "AI-generated" }] },
      { name: "Stripe webhook handler", desc: "Processes Stripe webhook events for payment confirmations and refunds.", checklist: [], gaps: [], files: [], qs: [{ q: "Which webhook events are critical vs optional?", from: "Coworker" }] },
      { name: "Refund reconciliation job", desc: "Nightly batch job that reconciles refund records between Stripe and the internal ledger.", checklist: [], gaps: [], files: [], qs: [] },
      { name: "Currency conversion rates", desc: "Caches daily exchange rates from the ECB API. Refreshes at 04:00 UTC.", checklist: [{ text: "Fallback to previous rate on API failure", done: true }], gaps: [], files: [], qs: [] },
    ]},
    { name: "CI/CD Pipeline", cards: 8, qs: 3, gaps: 1, moduleGaps: ["Deployment process referenced in both Payment Service and CI/CD cards but described differently"], items: [
      { name: "Atlas migration rollback", desc: "MongoDB Atlas migration procedures and rollback steps.", checklist: [], gaps: ["Missing description detail"], files: [], qs: [{ q: "What's the rollback procedure for failed Atlas migrations?", from: "Hà Vy" }] },
      { name: "GitHub Actions workflow", desc: "Main CI workflow: lint → test → build → deploy. Uses matrix strategy for Node 18/20.", checklist: [{ text: "Staging deploy automated", done: true }, { text: "Prod deploy requires approval", done: true }], gaps: [], files: [], qs: [{ q: "Is there a runbook for the nightly batch job failures?", from: "Coworker" }] },
      { name: "Docker image caching", desc: "Layer caching strategy for Docker builds. Cache stored in GitHub Container Registry.", checklist: [], gaps: [], files: [], qs: [] },
    ]},
    { name: "Shared Libraries", cards: 6, qs: 2, gaps: 0, items: [
      { name: "API key rotation", desc: "Scheduled rotation of API keys across services. Runs every 90 days via GitHub Action.", checklist: [{ text: "Rotation schedule documented", done: true }, { text: "Auto-rotation configured", done: true }], gaps: [], files: [{ name: "api-key-rotation.md", size: "1.8 KB" }], qs: [{ q: "Where is the API key rotation runbook?", from: "AI-generated" }] },
      { name: "Logging middleware", desc: "Shared Express middleware for structured JSON logging. Includes request ID propagation and PII redaction.", checklist: [], gaps: [], files: [], qs: [] },
      { name: "Auth token validator", desc: "JWT validation library shared across microservices. Supports RS256 and EdDSA.", checklist: [], gaps: [], files: [], qs: [{ q: "What's the token refresh strategy?", from: "AI-generated" }] },
    ]},
  ]},
  { board: "Platform Infrastructure", boardCards: 18, modules: [
    { name: "Monitoring & Alerts", cards: 10, qs: 3, gaps: 1, moduleGaps: ["No cards document the SLA commitments tied to these alert thresholds"], items: [
      { name: "Datadog dashboard setup", desc: "Main service health dashboard with SLO tracking. Monitors P50/P95/P99 latency.", checklist: [], gaps: ["Alert thresholds not documented"], files: [], qs: [{ q: "What are the critical alert thresholds?", from: "AI-generated" }] },
      { name: "PagerDuty escalation policy", desc: "On-call rotation and escalation paths. Primary → secondary → engineering manager.", checklist: [{ text: "Rotation schedule current", done: true }], gaps: [], files: [{ name: "oncall-schedule.pdf", size: "45 KB" }], qs: [] },
      { name: "Log aggregation pipeline", desc: "Fluentd → Elasticsearch → Kibana stack for centralized logging.", checklist: [], gaps: [], files: [], qs: [{ q: "What's the log retention policy?", from: "AI-generated" }] },
    ]},
    { name: "Infrastructure as Code", cards: 8, qs: 2, gaps: 0, items: [
      { name: "Terraform modules", desc: "Reusable Terraform modules for VPC, EKS, RDS. State stored in S3 with DynamoDB locking.", checklist: [], gaps: [], files: [], qs: [{ q: "Which Terraform version and providers are pinned?", from: "AI-generated" }] },
      { name: "Helm chart templates", desc: "Standardized Helm charts for service deployments. Includes sidecar injection for monitoring.", checklist: [], gaps: [], files: [], qs: [{ q: "How do we handle Helm value overrides per environment?", from: "AI-generated" }] },
      { name: "Secrets management", desc: "HashiCorp Vault integration for runtime secrets. Services use sidecar injector.", checklist: [{ text: "Vault auto-unseal configured", done: true }, { text: "Secret rotation automated", done: false }], gaps: [], files: [], qs: [] },
    ]},
  ]},
];
...
function SessionPage({ role, stepId }) {
  ...
  return (<div className="max-w-5xl mx-auto p-6">
    ...
    {activeTab === "overview" && <OverviewContent role={role} stepId={stepId} isReady={isReady} onSwitchTab={setActiveTab} />}
...
function HeroBar({ phase }) {
  ...
  return (<div className="rounded-lg border border-gray-200 bg-white p-4 mb-5 flex items-center gap-4"><div className="w-12 h-12 rounded-full bg-violet-100 text-violet-700 text-sm font-semibold inline-flex items-center justify-center shrink-0">{s.initials}</div><div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-0.5"><h1 className="text-lg font-semibold text-gray-900">{s.name}&apos;s session</h1><span className={`text-[9px] px-2 py-0.5 rounded uppercase tracking-wider font-semibold border ${phaseCls}`}>{phaseLabel}</span></div><p className="text-[12px] text-gray-500">{s.role} · {s.dept}</p><p className="text-[11px] text-gray-500 mt-0.5" style={{ fontFamily: "ui-monospace, Menlo, monospace" }}>{s.daysLeft} days left · Deadline {s.deadline} · {metrics}</p></div></div>);
}

function OverviewContent({ role, stepId, isReady, onSwitchTab }) { if (role === "offboarder") return <OffboarderOverview stepId={stepId} />; if (role === "coworker") return <CoworkerOverview isReady={isReady} onSwitchTab={onSwitchTab} />; return <ManagerOverview stepId={stepId} isReady={isReady} onSwitchTab={onSwitchTab} />; }

function ManagerOverview({ stepId, isReady, onSwitchTab }) {
  ...
  return (<div className="space-y-4"><div className="rounded-lg border border-gray-200 bg-white p-5"><h3 className="text-sm font-semibold text-gray-900 mb-3">Data collection complete</h3><div className="grid grid-cols-4 gap-3 mb-4"><MetricCard label="Boards processed" value={SESSION.boards} /><MetricCard label="Cards eligible" value={SESSION.cards} /><MetricCard label="Knowledge areas" value={SESSION.modules} /><MetricCard label="Questions generated" value={SESSION.questions} /></div><div className="pt-3 border-t border-gray-100 space-y-2"><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium">Knowledge areas found</p><div className="flex flex-wrap gap-1.5">{["Payment Service", "CI/CD Pipeline", "Shared Libraries", "Monitoring & Alerts", "Infrastructure as Code"].map(m => (<span key={m} className="text-[11px] px-2 py-1 rounded-md bg-gray-50 border border-gray-200 text-gray-700">{m}</span>))}</div></div><div className="pt-3 mt-3 border-t border-gray-100 grid grid-cols-2 gap-3"><div><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1">Coworker engagement</p><p className="text-[12px] text-gray-700">2 of 3 coworkers have asked questions</p><p className="text-[11px] text-gray-500">1 coworker hasn&apos;t engaged yet</p></div><div><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1">Knowledge gaps</p><p className="text-[12px] text-gray-700">6 gaps detected</p><p className="text-[11px] text-gray-500">3 card-level · 3 AI-detected cross-card</p></div></div></div><div className="flex items-center gap-3"><button onClick={() => onSwitchTab("data")} className="h-9 px-4 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">Review in Data tab</button><Link href="/session/minh-le" className="h-9 px-4 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium inline-flex items-center gap-2 transition-colors">Start Capture<ArrowRight className="w-3.5 h-3.5" /></Link></div><p className="text-[10px] text-gray-400">Coworkers have been notified and can add questions. You can review the Data tab before starting Capture.</p></div>);
}
...
function CoworkerOverview({ isReady, onSwitchTab }) {
  ...
  return (<div className="space-y-4"><div className="rounded-lg border border-gray-200 bg-white p-5"><h3 className="text-sm font-semibold text-gray-900 mb-1">Minh Lê is leaving soon</h3><p className="text-[12px] text-gray-500 mb-3">Senior Backend Engineer · Engineering · Last day July 4, 2026</p><p className="text-xs text-gray-500 mb-4">Browse their knowledge areas below and ask questions about what you&apos;ll need after they&apos;re gone. Questions can be added anytime, even after Capture starts.</p><div className="pt-3 border-t border-gray-100 space-y-2"><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium">Knowledge areas available</p><div className="flex flex-wrap gap-1.5">{["Payment Service", "CI/CD Pipeline", "Shared Libraries", "Monitoring & Alerts", "Infrastructure as Code"].map(m => (<span key={m} className="text-[11px] px-2 py-1 rounded-md bg-gray-50 border border-gray-200 text-gray-700">{m}</span>))}</div></div><div className="pt-3 mt-3 border-t border-gray-100 grid grid-cols-2 gap-3"><div><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1">Your activity</p><p className="text-[12px] text-gray-700">You&apos;ve asked 0 questions</p><p className="text-[11px] text-gray-500">Browse the Data tab to ask</p></div><div><p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1">Other coworkers</p><p className="text-[12px] text-gray-700">2 others are asking questions</p><p className="text-[11px] text-gray-500">{SESSION.questions} questions collected so far</p></div></div></div><button onClick={() => onSwitchTab("data")} className="h-9 px-4 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium inline-flex items-center gap-2 transition-colors">Browse Data tab<ArrowRight className="w-3.5 h-3.5" /></button></div>);
}
...
function DataContent({ role, stepId, isReady }) {
  ...
  return (<div className="relative"><div>
    ...
    {GENERAL_QUESTIONS.length > 0 && (<div className="rounded-lg border border-violet-200 bg-violet-50/20 mb-3 overflow-hidden"><div className="px-4 py-2.5 bg-violet-50/40 border-b border-violet-200 flex items-center gap-2"><MessageCircle className="w-3.5 h-3.5 text-violet-500" /><span className="text-sm font-semibold text-gray-900">General questions</span><span className="text-[11px] text-gray-500">{GENERAL_QUESTIONS.length} questions not tied to a specific card</span></div>{GENERAL_QUESTIONS.map((q, i) => (<div key={i} className="px-4 py-2.5 border-b border-violet-100 last:border-b-0 flex items-start gap-2"><div className="flex-1"><div className="text-[12px] text-gray-900">{q.q}</div><div className="text-[10px] text-gray-500 flex items-center gap-1 mt-0.5"><User className="w-2.5 h-2.5" />{q.from}</div></div></div>))}</div>)}
...
function ModuleSection({ mod, role, selectedCard, onSelectCard }) {
  ...
  return (<div className="border-b border-gray-100 last:border-b-0">
    <button onClick={() => setExpanded(!expanded)} className="w-full px-4 py-2 flex items-center justify-between text-left hover:bg-gray-50 transition-colors"><div className="flex items-center gap-2"><ChevronDown className={`w-3 h-3 text-gray-400 transition-transform ${expanded ? "" : "-rotate-90"}`} /><span className="text-[13px] font-medium text-gray-900">{mod.name}</span><span className="text-[11px] text-gray-500">{mod.cards} cards</span>{mod.qs > 0 && <span className="text-[9px] px-1.5 py-0.5 rounded bg-violet-50 text-violet-600 border border-violet-200">{mod.qs} Qs</span>}{(mod.gaps + (mod.moduleGaps || []).length) > 0 && <span className="text-[9px] px-1.5 py-0.5 rounded bg-yellow-50 text-yellow-700 border border-yellow-200">{mod.gaps + (mod.moduleGaps || []).length} gap{mod.gaps + (mod.moduleGaps || []).length > 1 ? "s" : ""}</span>}</div>{role === "manager" && <span className="flex items-center gap-2 text-[10px] text-gray-400"><span className="hover:text-violet-600 inline-flex items-center gap-0.5"><Upload className="w-2.5 h-2.5" />Upload</span><span className="hover:text-violet-600">Rename</span></span>}</button>
    {expanded && (<>
      {mod.moduleGaps && mod.moduleGaps.length > 0 && (<div className="px-4 py-2 pl-10 border-t border-gray-50 space-y-1">{mod.moduleGaps.map((g, gi) => (<div key={gi} className="text-[10px] text-yellow-800 bg-yellow-50 border border-yellow-200 rounded-md px-2.5 py-1.5 flex items-start gap-1.5"><Sparkles className="w-3 h-3 shrink-0 mt-0.5 text-yellow-600" /><div><span>{g}</span><span className="text-[9px] text-yellow-600 ml-1">· AI-detected across cards</span></div></div>))}</div>)}
      {mod.items && mod.items.map((card, ci) => { const isSel = selectedCard?.name === card.name; return (<button key={ci} onClick={() => onSelectCard(card)} className={`w-full px-4 py-2 pl-10 flex items-center gap-2 text-left border-t border-gray-50 transition-colors ${isSel ? "bg-violet-50 border-l-2 border-l-violet-500" : "hover:bg-gray-50"}`}><FileText className="w-3 h-3 text-gray-400 shrink-0" /><span className="text-[12px] text-gray-800 flex-1">{card.name}</span>{card.files && card.files.length > 0 && <Paperclip className="w-2.5 h-2.5 text-gray-300" />}{card.gaps.length > 0 && <span className="text-[8px] px-1 py-0.5 rounded bg-yellow-50 text-yellow-700">gap</span>}{card.qs.length > 0 && <span className="text-[8px] px-1 py-0.5 rounded bg-violet-50 text-violet-600">{card.qs.length} Q</span>}</button>); })}{(!mod.items || mod.items.length === 0) && (<div className="px-4 py-2 pl-10 text-[11px] text-gray-400 border-t border-gray-50">{mod.cards} cards — click to expand in full build</div>)}{role !== "offboarder" && (<div className="px-4 py-1.5 pl-10 border-t border-gray-50"><button onClick={(e) => { e.stopPropagation(); setShowModQ(!showModQ); }} className="text-[10px] text-violet-600 inline-flex items-center gap-1 hover:text-violet-700"><Plus className="w-2.5 h-2.5" />Ask about this module</button>{showModQ && <div className="flex gap-1.5 mt-1.5"><input placeholder={`Question about ${mod.name}...`} className="flex-1 h-7 px-2 rounded border border-gray-200 text-[10px] focus:outline-none focus:ring-2 focus:ring-violet-500/20" /><button className="h-7 px-2 rounded bg-violet-600 text-white text-[9px]">Ask</button></div>}</div>)}</>)}
  </div>);
}
...
function SidePanel({ card, role, onClose, isCapture }) {
  ...
  return (<div className="p-5">
    ...
    <p className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mt-3 mb-1 pt-2 border-t border-gray-100">Attachments ({(card.files || []).length})</p>
    {(card.files || []).length > 0 ? (card.files || []).map((f, i) => (<div key={i} className="flex items-center gap-2 text-[11px] py-1.5 px-2.5 rounded-md bg-gray-50 mb-1"><Paperclip className="w-3 h-3 text-gray-400 shrink-0" /><span className="text-gray-800 flex-1">{f.name}</span><span className="text-[10px] text-gray-400">{f.size}</span></div>)) : <p className="text-[11px] text-gray-400 mb-1">No files attached yet</p>}
    {role !== "offboarder" && <button className="text-[10px] text-violet-600 inline-flex items-center gap-1 mt-1 hover:text-violet-700"><Upload className="w-2.5 h-2.5" />Upload a file</button>}
    ...
    {card.qs.length > 0 ? card.qs.map((q, i) => (<div key={i} className="text-[11px] bg-gray-50 rounded-md px-2.5 py-2 mb-1.5"><p className="text-gray-900 mb-0.5">{q.q}</p><p className="text-[10px] text-gray-500 flex items-center gap-1">{q.from === "AI-generated" ? <><Sparkles className="w-2.5 h-2.5 text-violet-500" />{q.from}</> : <><User className="w-2.5 h-2.5" />{q.from}</>}</p>{isCapture && role === "offboarder" && (<div className="mt-2 pt-2 border-t border-gray-200"><textarea placeholder="Type your answer..." className="w-full h-16 px-2 py-1.5 rounded border border-gray-200 text-[11px] resize-none focus:outline-none focus:ring-2 focus:ring-violet-500/20" /><div className="flex items-center justify-between mt-1"><button className="text-[10px] text-gray-500 inline-flex items-center gap-1 hover:text-violet-600"><Paperclip className="w-2.5 h-2.5" />Attach file</button><button className="h-6 px-2 rounded bg-violet-600 text-white text-[10px]">Submit answer</button></div></div>)}</div>)) : <p className="text-[11px] text-gray-400 mb-1">No questions on this card yet</p>}
...
function LogsContent({ role, stepId }) {
  if (stepId === "collecting" || (role === "offboarder" && stepId !== "capture-preview")) return <div className="text-center py-8 text-sm text-gray-500">No activity yet</div>;
  const logs = stepId === "capture-preview" ? [
    { ts: "11:05 AM", type: "questions", text: "Minh Lê answered: How does the retry logic handle poison messages?" },
    { ts: "10:58 AM", type: "system", text: "Capture started — Minh Lê notified" },
    { ts: "10:45 AM", type: "questions", text: "Coworker asked: Which webhook events are critical vs optional?" },
    ...
    { ts: "10:45 AM", type: "questions", text: "Coworker asked a general question: What's the process for handling customer escalations?" },
    { ts: "10:40 AM", type: "questions", text: "Hà Vy asked: What's the rollback procedure for failed Atlas migrations?" },
    { ts: "10:32 AM", type: "system", text: "Crawl complete — 3 boards, 64 eligible cards, 5 modules derived" },
  ...
  return (<div className="space-y-1.5"><div className="flex gap-2 mb-3">{["All", "System", "Questions", "Files", "Edits"].map(f => (<button key={f} className={`px-2.5 py-1 rounded-md text-[11px] ${f === "All" ? "bg-violet-50 text-violet-700 font-medium" : "text-gray-500 hover:bg-gray-100"}`}>{f}</button>))}</div>{logs.map((l, i) => { const border = l.type === "system" ? "rgb(229,231,235)" : l.type === "questions" ? "rgb(124,58,237)" : "rgb(234,179,8)"; return (<div key={i} className="flex items-start gap-3 px-3 py-2 rounded-md border border-gray-200 bg-white" style={{ borderLeft: `2px solid ${border}` }}><span className="text-[10px] text-gray-500 shrink-0 mt-0.5" style={{ fontFamily: "ui-monospace, Menlo, monospace" }}>{l.ts}</span><span className={`text-[9px] px-1.5 py-0.5 rounded uppercase tracking-wider font-medium shrink-0 ${l.type === "questions" ? "bg-violet-50 text-violet-600" : "bg-gray-100 text-gray-500"}`}>{l.type}</span><span className="text-[11px] text-gray-900">{l.text}</span></div>); })}</div>);
}
