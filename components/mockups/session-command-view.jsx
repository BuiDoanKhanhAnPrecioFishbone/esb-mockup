...
function ModuleSection({ mod, role, isCapture, isDeliver, isComplete, canEditQs, primaryCards=[], linkedCards=[], showProgress, addedQs, onAddModQ, onEditModQ, onDeleteModQ, canManage, dismissedFlags, onDismissFlag, onMoveCard, primaryModuleOf, selectedCard, onSelectCard }) {
  ...
  return <div className="border-b border-gray-100 last:border-b-0">
    ...
    {expanded&&<>
      ...
      {moduleGaps.length>0&&<div className="px-4 py-2 pl-10 border-t border-gray-50 space-y-1.5">{moduleGaps.map((g,gi)=><div key={gi} className="rounded-md bg-yellow-50 border border-yellow-200 px-2.5 py-1.5"><div className="text-[10px] text-yellow-800 flex items-start gap-1.5"><Sparkles className="w-3 h-3 shrink-0 mt-0.5 text-yellow-600"/>{g}</div>{moduleGapQs[gi]&&<div className="text-[10px] text-violet-700 flex items-start gap-1.5 mt-1.5 pl-4 border-t border-yellow-200/70 pt-1.5"><HelpCircle className="w-3 h-3 shrink-0 mt-0.5 text-violet-500"/><span><span className="text-violet-500 font-medium">AI question:</span> {moduleGapQs[gi]}</span></div>}</div>)}</div>}
