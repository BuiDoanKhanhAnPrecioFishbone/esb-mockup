...
/* Session Command View — CL-119/120/127/128/129
   ...
   Module headers: Rename only. No upload in POC.
...
function CoworkerNetwork({ coworkers=[], onAdd, onRemove, readOnly=false }) {
  ...
  return <div className="rounded-lg border border-gray-200 bg-white overflow-hidden">
    <div className="px-4 py-2.5 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
      ...
      {!readOnly&&onAdd&&<button onClick={()=>setShowAdd(!showAdd)} className="text-[10px] px-2 py-1 rounded-md border border-violet-200 text-violet-700 bg-violet-50 hover:bg-violet-100 cursor-pointer font-medium">+ Add</button>}
    </div>
    {coworkers.map(cw=><div key={cw.id} className="px-4 py-2.5 border-b border-gray-100 last:border-b-0 flex items-center gap-3">
      ...
      {!readOnly&&cw.source==="manual"&&onRemove&&<button onClick={()=>onRemove(cw.id)} className="w-5 h-5 rounded hover:bg-rose-50 inline-flex items-center justify-center text-gray-300 hover:text-rose-500 cursor-pointer shrink-0 text-[14px]">\u00d7</button>}
    </div>)}
