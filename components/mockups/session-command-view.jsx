...
function SidePanel({ card, role, onClose, isCapture, isDeliver, isComplete, primaryModule, sat, onOpenReasoning }) {
  ...
  return <div className="p-5">
    ...
    <div className="flex flex-wrap gap-1.5 mb-1">{(()=>{const cls=classify(card);const open=onOpenReasoning?()=>onOpenReasoning(card):undefined;
      ...
      return <><button onClick={open} className="text-[10px] px-2 py-0.5 rounded border border-violet-200 bg-violet-50 text-violet-700 font-medium inline-flex items-center gap-1 cursor-pointer"><span className="text-violet-500">★</span>{primaryModule}{open&&<ChevronRight className="w-2.5 h-2.5 text-violet-400"/>}</button>{linkedMods.map(m=><span key={m} className="text-[10px] px-2 py-0.5 rounded bg-white text-violet-600 border border-violet-200 border-dashed inline-flex items-center gap-0.5"><ArrowUpRight className="w-2.5 h-2.5"/>{m}</span>)}</>;
    })()}</div>
