...
function SessionPage({ role, stepId, initialTab }) {
  ...
  const tabs = role==="offboarder"
    ? [{id:"overview",label:"Overview"},{id:"data",label:"Data",hidden:true},{id:"logs",label:"Logs",disabled:true,tip:"Not available in your view"}]
    : [{id:"overview",label:"Overview"},{id:"data",label:"Data",disabled:!dataLogsActive,tip:tabTip},{id:"logs",label:"Logs",disabled:!dataLogsActive,tip:tabTip}];
...
// Orbital illustration (R3-01) — the departure-pending / "graph is building" signal,
// mirroring the dashboard zero-state. Kept local to avoid a circular import.
...
function ManagerOverview({ stepId, isReady, onSwitchTab, coworkers, onAddCoworker, onRemoveCoworker }) {
  if (!isReady) return <div className="space-y-4"><div className="rounded-xl border border-gray-200 bg-white p-8 text-center"><OrbitalIllustration/><h3 className="text-sm font-semibold text-gray-900 mb-1">{"Collecting data from "}{SESSION.boards}{" boards\u2026"}</h3><p className="text-xs text-gray-500">{"Building "}{SESSION.name}{"\u2019s knowledge graph. We\u2019ll notify you when it\u2019s ready."}</p></div><CategorizeAnimation/></div>;
...
function GapContextPanel({ moduleName, onClose }) {
  ...
  return <div className="p-5">
    ...
    <button className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md bg-violet-50 border border-violet-200 text-violet-700 text-[11px] font-medium hover:bg-violet-100 cursor-pointer"><Sparkles className="w-3 h-3"/>Ask about this gap</button>
  </div>;
}
...
function SidePanel({ card, role, onClose, isCapture, isDeliver, isComplete, primaryModule }) {
