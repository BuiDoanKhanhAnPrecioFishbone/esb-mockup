...
function SessionActionsMenu({ stepId }) {
  const [open, setOpen] = useState(false);
  const [confirm, setConfirm] = useState(false);
  const [cancelled, setCancelled] = useState(false);
  const [backTo, setBackTo] = useState(null);       // "Prepare" | "Capture" — pending backward confirm
  const [movedBack, setMovedBack] = useState(null); // toast target after moving back
  const back = stepId==="capture" ? "Prepare" : stepId==="deliver" ? "Capture" : null;
  return <div className="relative shrink-0">
    ...
    {open&&<div className="absolute right-0 top-full mt-1 z-30 w-44 rounded-md border border-gray-200 bg-white shadow-lg py-1">
      {back&&<button onMouseDown={e=>e.preventDefault()} onClick={()=>{setOpen(false);setBackTo(back);}} className="w-full text-left px-3 py-1.5 text-[12px] text-gray-700 hover:bg-gray-50 cursor-pointer inline-flex items-center gap-1.5"><ArrowRight className="w-3 h-3 rotate-180"/>{"Back to "}{back}</button>}
      <button onMouseDown={e=>e.preventDefault()} onClick={()=>{setOpen(false);setConfirm(true);}} className="w-full text-left px-3 py-1.5 text-[12px] text-rose-600 hover:bg-rose-50 cursor-pointer">Cancel session</button>
    </div>}
    {backTo&&<div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center" onClick={()=>setBackTo(null)}><div className="bg-white rounded-xl shadow-xl p-6 w-[380px] border border-gray-200" onClick={e=>e.stopPropagation()}><h3 className="text-base font-semibold mb-2">{"Move back to "}{backTo}{"?"}</h3><p className="text-[12px] text-gray-500 mb-4">{"The session reopens for the previous stage. Minh Lê will be notified that more input is needed. You can move forward again when ready."}</p><div className="flex gap-2 justify-end"><button onClick={()=>setBackTo(null)} className="h-8 px-3 rounded-md border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50 cursor-pointer">Stay here</button><button onClick={()=>{setMovedBack(backTo);setBackTo(null);}} className="h-8 px-3 rounded-md bg-violet-600 text-white text-sm font-medium hover:bg-violet-700 cursor-pointer">{"Move to "}{backTo}</button></div></div></div>}
    ...
    {movedBack&&<div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 rounded-lg bg-gray-900 text-white text-[12px] px-4 py-2.5 shadow-lg flex items-center gap-2"><RotateCcw className="w-4 h-4 text-violet-300"/>{"Session moved back to "}{movedBack}{" — Minh Lê notified."}</div>}
  </div>;
}

// PH-01/02 — 3-step stepper (circles + connectors). The forward CTA is passed in; no summary text (PH-02).
function PhaseHero({ phase, cta }) {
  ...
  const circle = (state) => state==="completed"
    ? <span className="w-5 h-5 rounded-full bg-emerald-500 inline-flex items-center justify-center shrink-0"><Check className="w-3 h-3 text-white"/></span>
    : state==="active"
    ? <span className="w-5 h-5 rounded-full bg-violet-600 inline-flex items-center justify-center shrink-0"><span className="w-1.5 h-1.5 rounded-full bg-white"/></span>
    : <span className="w-5 h-5 rounded-full border-2 border-gray-300 bg-white inline-block shrink-0"/>;
  return <div className="rounded-lg border border-violet-200 bg-violet-50/40 p-5">
    <div className="flex items-center">{steps.map((s,i)=>{
      const state = i<activeIdx?"completed":i===activeIdx?"active":"upcoming";
      const labelCls = state==="completed"?"text-gray-500":state==="active"?"font-semibold text-violet-700":"text-gray-300";
      const connColor = state==="completed"?"#059669":state==="active"?"#c4b5fd":"#e5e7eb";
      return <React.Fragment key={s.id}>
        <div className="flex items-center gap-1.5 shrink-0">{circle(state)}<span className={`text-[11px] ${labelCls}`}>{s.label}</span></div>
        {i<steps.length-1&&<div className="flex-1 mx-2 border-t-2" style={{borderColor:connColor,borderStyle:state==="completed"?"solid":"dashed"}}/>}
      </React.Fragment>;
    })}</div>
    {cta&&<div className="flex justify-center mt-4">{cta}</div>}
  </div>;
}
...
function DataContent({ role, stepId, isReady, canEditQs, generalQs, addedModQs, onAddGQ, onEditGQ, onDeleteGQ, onAddModQ, onEditModQ, onDeleteModQ, focusQ, focusKey }) {
  ...
  const satCtl = { satKeys, moreKeys, undoneKeys, onSatisfyQ, onMoreQ, onUndoQ };
