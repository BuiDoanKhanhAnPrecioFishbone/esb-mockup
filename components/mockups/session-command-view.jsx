...
// PH-01/02 + UI-01 — compact 3-step stepper (16px circles, 10px labels) with the forward CTA
// tucked directly beneath it (~80px total, one component). No summary text (PH-02).
function PhaseHero({ phase, cta }) {
  ...
  return <div className="rounded-lg border border-violet-200 bg-violet-50/40 px-4 py-2.5">
    <div className="flex items-center">{steps.map((s,i)=>{
      ...
      return <React.Fragment key={s.id}>
        <div className="flex items-center gap-1 shrink-0">{circle(state)}<span className={`text-[10px] ${labelCls}`}>{s.label}</span></div>
        {i<steps.length-1&&<div className="flex-1 mx-1.5 border-t-2" style={{borderColor:connColor,borderStyle:state==="completed"?"solid":"dashed"}}/>}
      </React.Fragment>;
    })}</div>
    {cta&&<div className="flex justify-center mt-2">{cta}</div>}
  </div>;
}
