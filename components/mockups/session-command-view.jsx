...
function ManagerOverview({ stepId, isReady, onSwitchTab }) {
  if (!isReady) return <div className="rounded-lg border border-gray-200 bg-white p-6 text-center"><div className="w-10 h-10 rounded-full bg-violet-50 inline-flex items-center justify-center mb-3 mx-auto"><div className="w-4 h-4 rounded-full border-2 border-gray-300 border-t-violet-500 animate-spin"/></div><h3 className="text-sm font-medium text-gray-900 mb-1">{"Collecting data from "}{SESSION.boards}{" boards..."}</h3><p className="text-xs text-gray-500">{"\u2019ll notify you when ready."}</p></div>;
