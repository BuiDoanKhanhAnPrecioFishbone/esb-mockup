...
Claude will ask for one. Open the **live app** (the password-protected URL), navigate to that
exact screen, and take a normal screenshot. Paste it into the chat. That's the picture Claude
uses to get the positions right — Claude can't open the app itself, so this one capture is on
you (takes 5 seconds). Make sure the app is showing the *same* step/role/tab you asked for.
