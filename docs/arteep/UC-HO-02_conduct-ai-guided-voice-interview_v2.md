...
|-------------------------|-------------------------------------------------------|-|-|
| **Use Case Name:**      | Conduct AI-Guided Voice Interview                     | **Version:** | 2.0 |
| **Created By:**         |                    | **Last Updated By:**   | BA Pod |
| **Date Created:**       |                    | **Date Last Updated:** | Sprint 2 |

> **Version 2.0 changes:** Replaces the hardcoded 4-section domain coverage (Red Flags / Unwritten Rules / Project Map / File Directory) with **dynamic N-domain coverage** derived from the Preliminary Knowledge Map and Manager's UC-HO-05 prompts — aligning UC-HO-02 with the Dynamic Content Clustering principle that governs UC-ON-01. Introduces ComplexityScore-based routing for question generation, GraphRAG dual-strategy retrieval, and live RBAC scope enforcement. See Change Log at end of document for full delta.
...
| **Assumptions:** | 1. The organization has integrated ART-EEP with Jira and Google Drive for context seeding. Email integration is optional. 2. Whisper API (or equivalent STT service) is provisioned and available in the deployment environment. 3. The Offboarder is a cooperative participant; the system does not handle adversarial or deliberately uninformative responses beyond flagging low-content segments. 4. Interviews are conducted in Vietnamese or English; the Handover AI Agent supports both languages in the same session. 5. The Semantic Kernel Planner Agent is configured with the ComplexityScore evaluation function, threshold values (low / high / confidence-gate), and the GraphRAG dual-strategy routing rules prior to go-live. 6. The minimum coverage threshold per knowledge domain (for the `Incomplete — Supplementary Review Suggested` flag at step 11) is configurable per organization. |
| **Notes and Issues:** | [TBD-1] Clarify whether the 45-minute target is a guideline or enforced — currently modeled as a soft warning at 40 min and hard stop at 60 min. Owner: Product Owner. Due: Sprint 2. [TBD-2] Define the fallback question bank for EX.4 (Low Context mode) — needs input from HR domain experts. Owner: BA / HR Lead. Due: Sprint 3. [TBD-3] Confirm data residency requirements for voice recordings if the organization operates across multiple jurisdictions. Owner: Legal / Infosec. Due: Sprint 2. [TBD-4] Define ComplexityScore threshold values (low / high / confidence-gate) per question type. Currently provisional; needs benchmarking against real handover transcripts. Owner: AI Engineer. Due: Sprint 3. [TBD-5] Define the minimum coverage threshold per knowledge domain for the `Incomplete` flag at step 11 (e.g., minutes spent, sub-questions answered, entities extracted). Owner: AI Engineer / BA. Due: Sprint 3. [TBD-6] Decide whether the Offboarder should see the domain coverage progress indicator (step 8) — could it create pressure to skim domains? Owner: Product Owner / UX. Due: Sprint 2. |
...
| C6 | Actor is a specific role, not "User" | ✅ | "Offboarder (departing employee)" with named secondary system actors including Worker/Expert agents |
| C7 | Description answers WHY + WHAT + OUTCOME | ✅ | Why: tacit knowledge can't be captured from docs. What: ≈45-min AI voice interview with Planner-driven dynamic domain coverage and ComplexityScore-routed question generation. Outcome: N-section draft summary ready for review |
...
| C10 | Postconditions cover success state and all system changes | ✅ | 5 postconditions including the full generation trace in audit log |
| C11 | Preconditions not confused with Assumptions | ✅ | Preconditions = hard system states; Assumptions = environmental / configuration beliefs |
...
| C16 | Each AC specifies "at step N" + condition | ✅ | All 4 ACs reference specific steps; new AC.4 covers confidence-gate escalation |
| C17 | Each Exception has trigger + system response + final state | ✅ | All 6 EXs follow the 3-part structure |
| C18 | Common failure modes covered | ✅ | Hardware failure (EX.1), API timeout (EX.2), inactivity (EX.3), missing seed data (EX.4), time limit (EX.5), full RBAC trim (EX.6) |
| C19 | Includes point to existing UCs | ✅ | UC-HO-01, UC-HO-03, and UC-HO-05 all exist in the UC List |
| C20 | Special Requirements don't duplicate functional requirements | ✅ | All SRs are non-functional (performance, security, retention, usability, dynamic domain coverage, token cost management, RBAC enforcement, audit completeness) |

**Result: 20/20 ✅ — UC-HO-02 v2.0 passes all quality checks.**
...
|---|---|---|
| 1.0 | Sprint 1 | Initial specification. Hardcoded coverage of 4 sections (Red Flags / Unwritten Rules / Project Map / File Directory). |
| **2.0** | **Sprint 2** | **Replaced hardcoded 4-section coverage with dynamic N-domain coverage derived from UC-HO-01 Preliminary Knowledge Map and UC-HO-05 Manager prompts (Description, step 2, step 8, step 11, SR). Named Semantic Kernel Orchestrator and Planner Agent explicitly throughout. Introduced ComplexityScore-based routing for question generation (step 5, step 7, SR). Added GraphRAG dual-strategy retrieval (Local + DRIFT) per question intent (step 7, SR). Added pre-retrieval ACL trimming via session RBAC scope established in UC-HO-01 (step 7, AC.3, SR, EX.6). Added AC.4 (confidence-gate Worker → Expert escalation). Added EX.6 (RBAC trim excludes retrieval context for a question). Added full generation trace to audit log as PC.5 (Planner decisions, routing, escalations, retrieval strategy). Added [TBD-4], [TBD-5], [TBD-6] to the open issues register.** |
