...
*Version 1.1 · BA Authoritative Reference · Consolidates all Use Cases across the Handover (HO) and Onboarding (ON) modules.*

*Updated 2026-06-05 (v1.1) · POC scope change: UC-HO-02 (voice interview) deferred to Phase 2; POC capture runs through self-serve upload + an asynchronous question queue. New **UC-HO-08** (Solicit Handover Inputs from the Employee's Network) added in the Prepare stage. See CL-098–101.*

---

## 0. Reading This Document

This index is the single navigation layer over the individual Use Case Specifications that define ART-EEP. It does not replace the full UC documents — it makes them addressable. Every claim in this index is traceable back to a numbered step, rule, or condition in one of the underlying UCs.

**Coverage note.** Nine UCs were provided in the original document set: HO-01, HO-03, HO-04, HO-05, HO-06, HO-07, ON-01, ON-02, ON-03. UC-HO-02 (*Conduct AI-Guided Voice Interview*) is referenced as a precondition, postcondition trigger, or include by six of the nine — but its full specification is not in this upload. Its entry in §2 is reconstructed from cross-references and flagged as such; the team should reconcile against the canonical UC-HO-02 spec when available.

**POC scope (2026-06-05).** For the POC, **UC-HO-02 (voice interview) is deferred to Phase 2**. Capture runs instead through the Offboarder's own file uploads plus an **asynchronous question queue** (manager prompts + network-solicited questions), answered in text at the Offboarder's own pace (CL-098 / CL-099). A new use case **UC-HO-08 (*Solicit Handover Inputs from the Employee's Network*)** is added in the Prepare stage: it notifies the offboarding employee's auto-derived connection set and collects both questions (which feed the capture queue) and flags on AI-collected data that's wrong or insufficient (which become correction requests for the Offboarder, CL-100 / CL-101). Both are reflected in §2, §5, and §8 below.

---

## 1. System Overview

ART-EEP is an Enterprise Knowledge Graph platform that converts the tacit knowledge of a departing employee (Offboarder) into structured, queryable institutional memory, then uses that memory to personalize the experience of a new employee (Onboarder) into the same role. The system operates through three coordinated capability layers — **Capture** (AI-guided voice interview in Phase 2; self-serve upload + question queue in the POC, plus human verification), **Commit** (GraphRAG indexing into Cosmos DB Gremlin + Azure AI Search), and **Consume** (personalized Playbook generation + interactive reading + skill gap analysis) — with a closed feedback loop (**Correct**) that allows any user to flag AI-generated content for Manager review and Active Learning refinement.

The Use Cases below decompose this end-to-end flow into individually testable, BA-validated specifications.
...
| **UC-HO-02** | Conduct AI-Guided Voice Interview | Handover | Offboarder | High | 5–20/month | ⏸ **Phase 2 — deferred (out of POC; replaced by upload + question queue)** |
...
| **UC-HO-05** | Configure Handover Interview Prompts | Handover | Manager | Medium | 0–3/session | ✅ Spec available (POC: prompts feed the capture queue) |
...
| **UC-HO-08** | Solicit Handover Inputs from the Employee's Network | Handover | System → Network (teammates, manager, coach) | High (POC) | 5–20/month | 🆕 Added 2026-06-05 (CL-100 / CL-101) |
...
### POC variant (2026-06-05)

For the POC, the **Capture** stage replaces the UC-HO-02 voice interview with two parallel, asynchronous inputs:

```
   UC-HO-01 ───► UC-HO-08  (Prepare · notify the auto-derived network)
   Initiate         Solicit network inputs
   Session          ├─ questions ──────────────┐
       │            └─ flags on AI-collected ──┐│
       │               data (CL-101)           ││
       ▼                                        ▼▼
   UC-HO-05  (Manager prompts) ───────►  Question queue + file uploads
   Configure                            (Offboarder answers async · CL-099)
                                                 │
                                                 ▼
                                          UC-HO-03  (review + sign)  ───►  UC-HO-04 …
```

UC-HO-02 (voice) is retained in the spec set for Phase 2 but is not on the POC critical path.
...
| Capture (POC) | HO-01, HO-08, HO-05, [upload + queue], HO-03 | 1–2 weeks | Review ≤3 business days (HO-03) |
...
The table below counts every appearance of each actor across all UCs, distinguishing primary (drives the use case) from secondary (supporting / notified / impacted).

| Actor | Primary In | Secondary In | Total Touchpoints |
|---|---|---|---|
| **Manager** | HO-01, HO-05, HO-07 | HO-03, HO-04, HO-06, HO-08, ON-01, ON-02, ON-03 | 10 |
| **Offboarder** | HO-02, HO-03, HO-06 | HO-04, HO-07, HO-08 (answers questions + resolves flags) | 6 |
| **Network (teammates / coach)** | HO-08 | — | 1 |
| **Onboarder** | HO-06, ON-02, ON-03 | ON-01 | 4 |
| **System (auto-triggered)** | HO-04, ON-01, HO-08 (notify + derive network) | HO-01, HO-02, HO-03, HO-05, HO-06, HO-07, ON-02, ON-03 | 11 |
...
| **Handover AI Agent** | Voice interview question generation (Phase 2); question-queue assembly (POC) | HO-02, HO-05, HO-08 |
| **Connection-graph deriver** | Auto-derive the offboarder's network from ingested data | HO-08 |
...
|---|---|---|---|
| UC-HO-01 | — (system entry point) | HO-08, HO-05, HO-02 *(Phase 2)* | — |
| UC-HO-02 *(Phase 2)* | HO-01 | HO-03 | HO-05 (live prompts via AC.3) |
| UC-HO-03 | HO-02 *(Phase 2)* **OR** [upload + question queue] *(POC)* | HO-04 (on approval) · re-capture (on rejection) | — |
| UC-HO-04 | HO-03 (status: `Approved`) | ON-01 | — |
| UC-HO-05 | HO-01 | — (modifies HO-02 queue / POC question queue) | — |
| UC-HO-06 | HO-04 (KG committed) **OR** ON-01 (Playbook generated) | HO-07 | — |
| UC-HO-07 | HO-06 | KG node update + Active Learning queue | — |
| UC-HO-08 | HO-01 | feeds the question queue (POC capture · CL-099) + pre-commit flags (CL-101) | HO-05 (network-extended prompts) |
...
> **(Phase 2)** HO-01 → HO-02 → HO-03 → HO-04 → ON-01 → ON-02
>
> **(POC · voice deferred)** HO-01 → HO-08 (solicit network inputs) → [Offboarder upload + answer question queue] → HO-03 → HO-04 → ON-01 → ON-02
...
The POC adds a **pre-commit** correction loop alongside it: **UC-HO-08 (colleague flag) → Offboarder corrects → content proceeds to HO-03 review** (CL-101). It is ACL-bounded — a colleague can only flag items they already had access to — and reuses the same "flagged — under review" grammar as the post-commit loop. Together these are what differentiate ART-EEP from a static document repository — the architectural proof point for the *Responsible AI* hackathon scoring criterion.
...
| Context seeding from shared workspaces (Trello in POC) | UC-HO-01 (referenced), UC-HO-05 AS.1 |
| Auto-derived connection graph for network solicitation | UC-HO-08 (CL-100) |
...
| Pre-commit, network-driven data flags | UC-HO-08 (CL-101) |
...
| 4-Layer Hard-Filter at ingestion · 0-token hover · prompt disambiguation | CL-091, CL-094 |

### 6.4 RBAC & Confidentiality
| Manifestation | UC References |
|---|---|
| AI Copilot scope restriction to role-relevant nodes | UC-ON-02 AS.3 |
| Pre-retrieval ACL trimming (Tier-1 stub exception, CL-093) | UC-ON-01 (architectural refinement) |
| Network flags ride existing ACLs — no net-new exposure | UC-HO-08 (CL-101) |
...
|---|---|---|---|
| 0. Flagged (pre-commit · POC) | `⚠️ Flagged — under review` | UC-HO-08 colleague flag (CL-101) | Connection set + Offboarder (within existing ACL) |
| 1. Captured | `Draft — Pending Offboarder Review` | UC-HO-02 (Phase 2) **OR** upload + question queue (POC) | Offboarder, Manager (read-only) |
...
All TBD items extracted from the UCs, sorted by sprint deadline.
...
| HO-05 [TBD-2] | ~~Decide if Manager prompts are visible to Offboarder pre-interview~~ → **RESOLVED 2026-06-05 (CL-099): yes — in the POC the prompts ARE the queue the Offboarder answers** | Product Owner / UX | UC-HO-05 |
...
| HO-06 [TBD-1] | ~~Confirm SLA for Manager correction review (default 5 business days)~~ → **RESOLVED 2026-06-05 (CL-095): 2 weekly cycles, then auto-escalate to the Critical path; sign-off still required** | BA / HR Lead | UC-HO-06 |
...
### New — UC-HO-08 (2026-06-05)
| ID | Description | Owner | Source |
|---|---|---|---|
| HO-08 [TBD-1] | Confirm how far the auto-derived connection set reaches (1-hop collaborators only, or N-hop) and the manager's edit window before requests send | Product Owner / BA | UC-HO-08 (CL-100) |
| HO-08 [TBD-2] | Confirm notification channel + reminder cadence for network requests | Product Owner / UX | UC-HO-08 |

**Note:** two prior TBDs (HO-05 TBD-2, HO-06 TBD-1) were resolved on 2026-06-05; two new HO-08 TBDs were opened.
...
| Multi-step task graph executes autonomously | UC-HO-04 steps 3–10 | 8-step pipeline from raw content to KG-indexed nodes — fully unattended |
| Routing decisions are signal-driven, not static | UC-ON-01 BR-01 (refined) | ComplexityScore evaluation at runtime |
| Context seeding extends LLM capability | UC-HO-01 (referenced), UC-HO-05 AS.1 | Shared-workspace ingestion (Trello in POC) with 4-Layer Hard-Filter |
| Network solicitation is auto-orchestrated | UC-HO-08 | Connection graph derived from ingested data; no manual list |
...
| The network corrects before commit | UC-HO-08 (CL-101) | Colleague flags wrong/insufficient data → Offboarder fixes, pre-commit |
| AI work is verifiable end-to-end | UC-HO-03 step 5, UC-ON-02 step 11 | "Source" chip linking every claim to its source segment |
| Disputed content is publicly marked | UC-HO-06 PC.2 | `⚠️ Disputed` / `flagged — under review` badge visible to readers during review |
...
| Cost designed out at every layer | CL-091, CL-093, CL-094, CL-101 | 4-Layer Hard-Filter, 0-token hover summaries, prompt disambiguation, token-free flag routing |
...
| **Connection set** | The offboarding employee's network (teammates, manager, coach), auto-derived from ingested data for UC-HO-08 (CL-100) |
...
| **Handover Record** | The complete bundle of verified content + entities + skills + summary committed in UC-HO-04 |
...
| **Question queue** | The POC capture mechanism (CL-099): the union of manager prompts (UC-HO-05) and network questions (UC-HO-08) that the Offboarder answers asynchronously in text |
...
Each individual UC carries its own 20-point quality checklist. All nine original UCs scored **20/20**. UC-HO-08 is newly added (2026-06-05) and pending its full spec + checklist. This index inherits that quality bar — every entry above is sourced from a passing UC or a logged CL.

| UC | Score | Validated |
|---|---|---|
| UC-HO-01 | (spec not in upload) | Pending |
| UC-HO-02 | (spec not in upload · Phase 2) | Pending |
| UC-HO-03 | 20/20 ✅ | Yes |
| UC-HO-04 | 20/20 ✅ | Yes |
| UC-HO-05 | 20/20 ✅ | Yes |
| UC-HO-06 | 20/20 ✅ | Yes |
| UC-HO-07 | 20/20 ✅ | Yes |
| UC-HO-08 | (new · spec pending) | Pending (CL-100 / CL-101) |
| UC-ON-01 | 20/20 ✅ | Yes |
| UC-ON-02 | 20/20 ✅ | Yes |
| UC-ON-03 | 20/20 ✅ | Yes |
...
**Product / UX:** UC-ON-02 (reading experience) → UC-ON-01 (Builder configuration) → UC-HO-08 (network solicitation, POC capture) → UC-ON-03 (Skill Gap UI) → UC-HO-03 (verification UX).

**Legal / Compliance:** UC-HO-03 (signature, retention) → UC-HO-04 (atomicity, audit) → UC-HO-07 (immutability) → UC-HO-08 (network-flag ACL boundary) → UC-ON-03 (auto-finalization implications).

**HR / People Ops:** UC-HO-01 (session initiation) → UC-HO-08 (network requests) → UC-HO-05 (Manager prompt steering) → UC-ON-03 (Skill Gap interpretation) → UC-HO-06/07 (correction governance).

**BA / QA:** This index, in order. Then each UC's 20-point checklist for verification.

---

*Master Index assembled across UC-HO-01 through UC-HO-08 and UC-ON-01 through UC-ON-03.*
