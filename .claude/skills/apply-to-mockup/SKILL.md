...
2. **Save a JSX artifact Claude generated in chat.** The user will usually say *"save this"*, *"put this on the site"*, or *"ship it"* after seeing the rendered artifact. Decide whether the artifact REPLACES content in an existing surface or becomes a NEW route.
...
1. **Read `CLAUDE.md`** at the repo root for the live route map and the file-routing cheat sheet.
...
3. **Identify which surface the request touches** and match it to the right file (cheat sheet — see `CLAUDE.md` for the canonical version):
